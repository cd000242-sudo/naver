...
**FINAL IMAGE FIX(2026-09-23 3차) 처리 결과**

| # | 상태 | 조치 |
|---|---|---|
| 1 | 고침 | `src/image/headingImageSelection.ts` 1-based 규칙 하나로 통일. 번호 출처 = sectionIndex → 글 소제목 목록 → 배치 순서, 모르면 유지. 메인 IPC·메인 다중계정·렌더러 다중계정 사전 필터에 적용. 설정으로 0장이면 성공(`filteredByMode`, `expectedCount`)으로 돌려주고 렌더러는 그 개수와 비교. 풀오토·반자동은 소제목 위치(sectionIndex)를 보내고 "설정상 없음"을 건너뜀으로 처리 |
| 2 | 고침 | `resolveImageRegenerateRoute` 하나로 그리드 🔄·프롬프트 카드 🔄·"🤖 AI 이미지 생성"·소제목 카드 재생성 통일. 고른 엔진 1회 호출, 네이버는 고를 때만, 나머지는 자동 대체 없이 안내 |
| 3 | 원인 수정 | 발행 때 오버레이는 v1.2.71 이후 꺼져 있었다(BlogExecutor가 설정을 전달하지 않음, 9/20~9/23 로그 0회). 실제 이중 원인은 "엔진이 제목을 그림 + 생성 때 오버레이". 문구 이름이 없는 썸네일은 ZERO TEXT로 그리게 했고, 썸네일마다 문구 상태를 붙였다. 발행 판단은 상태 기반·기본 거부 |
| 4 | 고침 | "🤖 AI 이미지 생성"이 한 번만 호출(2번과 같은 수정) |
| 5 | 그대로 | 이미지 스튜디오 우회 — 사용자가 직접 프롬프트를 쓰는 도구라 범위 밖 |
| 6 | 그대로 | 다양성 힌트 번호 — 범위 밖 |

발행 파이프라인(BlogExecutor)은 이미지 객체를 고정 필드로 다시 만들어 문구 상태가 발행 단계까지 가지 않는다. 발행 오버레이가 꺼져 있어 지금은 영향이 없다. 다시 켜려면 이 필드 전달과 `includeThumbnailText` 전달을 함께 넣어야 한다.
...
- **CARD_PROMISE.** 렌더러는 글 생성 결과의 클릭 사유(`preWritingAnalysis.clickReason`)를 CARD_PROMISE로 쓰고, 없으면 제목을 쓴다. `titleDiagnostics.ts` 주석은 파서가 그 값을 버린다고 적었지만, 그 진단은 모든 반환이 지나는 최종 결과물에서 값을 읽는다. 앱 로그 `main-2026-09-23.log`에 박서함 글의 `[TitleDiag] 클릭 사유: 안소희 팬심이 단순한 이름 언급이 아니라 '어머나'와 소희 양말로 이어지…`가 남아 있어, 값이 최종 결과물에 있다는 것을 확인했다. 주석이 낡았다.
