...
| T22 | 글쓰기 모드 / 이미지 전략 별도 UI | fullAutoWiring(발행 버튼 아래 한 줄·설정 모달 이미지 전략, 큐 패널에 중복 칸 없음 — v2.11.300) | PASS |
...
v2.11.299 는 기존 설정과 같은 역할의 칸을 새로 만들었고, 새 칸이 기존 설정을 조용히 덮어썼다(사장님 지적 2026-09-24).

- 연속발행 패널의 이미지 전략·소제목 범위·썸네일 문구 3선택과, 설정 창의 썸네일 문구 선택을 지웠다.
- 큐는 예전처럼 "⚡ 이미지 세팅하기"(소제목 전체/썸네일만/홀수만/짝수만)와 "썸네일 텍스트 포함"을 실행 때 읽는다. 행 표시도 같은 값을 보여 준다.
- 이미지 전략(홈판/피드 최적화 · 내 이미지 설정)은 "메인 풀오토 이미지 설정" 창 한 곳에만 둔다.
- 되돌림 확인: v2.11.299 코드에서 새 테스트("저장된 문구 빼기가 체크박스를 덮지 않는다")가 실패('none'), 수정 후 통과.
