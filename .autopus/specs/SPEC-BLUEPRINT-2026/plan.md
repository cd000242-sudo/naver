...
## Phase 2.5 · 비용 절감 (0.5일, 사장님 "비용이 사악하다")
- 캐시 정렬: OpenAI 자동 프롬프트 캐시(접두 1,024토큰 이상 동일)를 맞히도록 규칙부를 "글마다 같은 것 → 글마다 다른 것" 순으로 재배치(카테고리 오버레이·최근 노출글·서브키워드 블록을 재료 쪽으로). `[OpenAIDiag] cachedTokens` 로 적중률 측정. 목표: 본문 호출 입력 비용 $0.07 → ≤$0.02.
- 출력 메타 축소: 소제목별 summary·keywords·imagePrompt 와 제목 후보 설명(reasoning)을 스키마에서 빼거나 상한(각 20자)을 둔다. 이미지 프롬프트는 발행 시 별도 짧은 호출로. 목표: 출력 −35%.
- 소보정 경량화: 도입부·제목·소제목·인용 삽입 패치는 규칙 전문 대신 해당 계약 1~2KB만 붙인다. 목표: 패치 1회 $0.036 → ≤$0.006.
- 재생성 감소는 Phase 1~3 의 결과로 잰다.
- 측정: Phase 0 과 같은 16편 재실행, 편당 비용·캐시 적중률 표.
...
| 시각 | 항목 | 상태 | 커밋 |
|---|---|---|---|
| 02:32 | Phase 0 기준선 배치 시작(홈판 8·SEO 8, GPT-5.6 Terra, 진단 ON) | 진행 중 | — |
| 02:41 | Phase 1 설계도 모듈 5개 + 테스트 11건 | 완료 | 5ed077f6 |
| 02:47 | 비용 버그: 금지된 `[자료N]` 토큰을 세는 인용 밀도 기준 → 편마다 통과 불가능한 유료 재생성(기준선 6/6편) | 픽스 커밋, 빌드 대기 | 8862f6a2 |
| 02:54 | Phase 2 배선: 설계도 1회 → `[원본 텍스트]` 뒤 주입, 사실 목록 대체, 인용 후보 우선 | 코드 완료(vitest 8,674 GREEN·tsc 0), 빌드·실측 대기 | 8315c961 |

| 03:29 | 설계도 JSON 잘림(라우트 상한 2,048) → maxTokens 옵션·잘린 JSON 복구 | 커밋·빌드·라이브 확인(인용 4·사실 2) | 4255f744 |
| 03:36 | 비용 버그 2: 키워드 모드 압축률 50% 보존 재시도 → URL 모드 한정 | 커밋·빌드, 재측정 배치 03:38 시작 | 4de15b0c |

| 04:07 | Phase 4 재측정 1차: 편당 $0.200, 재생성 8/16 | 완료 | a715ce7a |
| 03:50 | 비용 버그 3: 가짜 환각 경고(누락·같은 방향) | 커밋·빌드 | 99ffc4e6 |
| 04:30 | 파서 보강: 안쪽 따옴표 복구·조각 투표 스냅 | 커밋·빌드 | 8184c1d3·11c646a3 |
| 04:44 | Phase 4 재측정 2차: 편당 $0.129, 재생성 0/16, 홈판 8/8 | 완료 | f14661b2 |
| 04:50 | 본문 길이 감소 대응: 설계도 최소선 문구 + factsMax 10 | 스팟체크 예정 | — |

| 04:55 | 스팟체크 4편: 미실행 2편 게이트 복귀(+268·+483자). 사장님: "본문 길이는 상관없다, 품질이 훌륭하면" → Phase 3 착수 지시 | 완료 | 08d6b42c |
| 10:20 | Phase 3 필드 바인딩: 도입부 패치에 readerSituation, 본문 인용 0 → 삽입 보정(짧은 호출) | 코드·테스트 7건·빌드, 라이브 스모크 2편 진행 | 9c2820a2 |

빌드는 배치가 dist 를 쓰는 동안 하지 않는다. 배치 종료 → `npm run build` → 앱 재시작 → Phase 4 재측정 배치(같은 16 키워드, `bash tmp/blueprint-baseline.sh after`) → 기준선과 나란히 보고.
