...
## Phase 2 — R2: Pre-publish Assertion (반쪽 발행 구조적 차단) — 예상 1~2일
...
## Phase 7 — 구조 개선 (안정화 안정권 진입 후, 장기)

- god file 분해: 발행부 → 상태 머신(`EDITING → PRE_PUBLISH_CHECK → PUBLISHING → PUBLISHED`) 도입. **characterization 테스트 선행 필수**
- 커버리지 목표: 발행 파이프라인 핵심 경로 20% (현재 3.7%)
- 셀렉터 원격 패치(remoteUpdate.ts) 운영 점검 — 네이버 UI 변경 무중단 대응
