...
export default function RpmTab({ onRadar }: { onRadar?: (pageUrl: string) => void }) {
    ...
    return (
        <>
            ...
            {/*
              * 사용법을 **탭 안에** 둔다(사장님 지시 2026-08-28 "사용 방법으로 놔둬 줄래").
              * 연결이 끝난 뒤에도 접힌 채로 남는다 — 계정을 바꾸거나 토큰이 만료되면
              * 다시 필요하고, 그때 딴 데서 찾게 두면 안 된다.
              */}
            <details className="lw-rpm-help">
                <summary>처음 한 번만 — 애드센스 연결하는 법</summary>
                <div className="lw-rpm-help-body">
                    <p>
                        RPM 은 <b>사장님 애드센스 계정의 실적</b>입니다. 그래서 구글 로그인이 한 번 필요합니다.
                        권한은 <code>adsense.readonly</code> 하나뿐이라 계정에 아무것도 바꾸지 못합니다.
                        토큰은 <b>이 PC 의 LEWORD 앱에만</b> 저장되고 사이트 서버로 나가지 않습니다.
                    </p>

                    <h4>1. AdSense API 켜기</h4>
                    <p>
                        위 <b>[① AdSense API 켜기]</b> 버튼을 누르고 <b>사용(Enable)</b> 을 클릭합니다.
                        누르기 전에 <b>왼쪽 위 프로젝트 이름</b>을 확인하세요 — 다른 프로젝트에 켜면 소용없습니다.
                        블로그스팟·유튜브를 쓰시던 프로젝트를 그대로 쓰면 됩니다.
                    </p>

                    <h4>2. 동의 화면 (이미 있으면 건너뜁니다)</h4>
                    <p>
                        User Type 은 <b>외부(External)</b>, 앱 이름·지원 이메일은 본인 것이면 됩니다.
                        범위(Scopes)는 추가하지 않아도 됩니다 — 로그인할 때 앱이 직접 요청합니다.
                    </p>

                    <h4>3. 게시 상태를 <b>프로덕션</b>으로 <span className="lw-rpm-warn">중요</span></h4>
                    <p>
                        동의 화면에서 <b>[앱 게시 / PUBLISH APP]</b> 을 누르세요.
                        <b>테스팅으로 두면 구글이 7일 뒤 갱신 토큰을 회수합니다</b> — 매주 다시 로그인해야 하고,
                        이 탭이 주기적으로 "연결이 만료됐습니다"를 냅니다.
                        프로덕션으로 올리면 로그인할 때 "확인되지 않은 앱" 경고가 뜨는데,
                        <b>[고급] → [(앱 이름)(으)로 이동]</b> 을 누르면 됩니다. 본인 앱에 본인이 로그인하는 것이라 검토 신청은 필요 없습니다.
                    </p>

                    <h4>4. 클라이언트 만들고 JSON 받기</h4>
                    <p>
                        위 <b>[② 데스크톱 앱 클라이언트 만들기]</b> 버튼을 누르고,
                        <b>애플리케이션 유형을 반드시 <code>데스크톱 앱</code></b> 으로 고릅니다. 이름은 아무거나(예: LEWORD).
                        만들면 <b>JSON 다운로드</b> 버튼이 나옵니다 — <code>client_secret_….json</code> 파일을 받아 두세요.
                    </p>
                    <p>
                        웹 애플리케이션으로 만들면 <code>127.0.0.1</code> 주소가 등록되지 않아 로그인이 막힙니다.
                        잘못 만드셔도 파일을 넣는 순간 이 화면이 알려 줍니다.
                    </p>

                    <h4>5. 넣고 로그인</h4>
                    <p>
                        위 <b>[③ 받은 JSON 넣기]</b> 에 받은 파일을 넣으면 두 칸이 자동으로 채워집니다.
                        <b>[구글 로그인]</b> 을 누르면 이 PC 에서 승인 창이 열리고, 승인하면 끝입니다.
                        <b>LEWORD 앱이 켜져 있어야</b> 합니다.
                    </p>

                    <h4>쓰는 법</h4>
                    <ul>
                        <li><b>[오늘]</b> — 방금 올린 글이 도는지 봅니다. 10분마다 알아서 다시 잽니다.</li>
                        <li><b>[추이]</b> — 그 글의 날짜별 RPM. 시작점이 낮으면 접고, 높게 시작하면 트래픽을 붓습니다. 나중에 오른 글은 언제부터 올랐는지가 보입니다.</li>
                        <li><b>[외부유입]</b> — 밀기로 정한 글을 레이더 탭으로 넘깁니다. 주소를 다시 넣지 않아도 됩니다.</li>
                        <li>뷰가 적을수록 값이 크게 흔들립니다. 100뷰 미만이면 몇 뷰짜리인지 함께 적힙니다.</li>
                    </ul>

                    <h4>막힐 때</h4>
                    <ul>
                        <li><b>403</b> — 1번을 안 했거나 다른 프로젝트에 켰습니다.</li>
                        <li><b>redirect_uri_mismatch</b> — 클라이언트 유형이 데스크톱 앱이 아닙니다.</li>
                        <li><b>로그인 창이 안 열림</b> — 앱이 꺼져 있거나 구버전입니다.</li>
                        <li><b>매주 만료됨</b> — 3번(프로덕션)을 안 하셨습니다.</li>
                        <li><b>표가 비어 있음</b> — 그 기간에 수익이 난 글이 없습니다. 게재 정지 중이면 그 기간은 0 입니다.</li>
                    </ul>
                </div>
            </details>
