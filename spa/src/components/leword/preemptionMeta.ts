...
export const WRITE_LANES: ReadonlyArray<{ id: string; label: string; hint: string }> = [
    ...
    /*
     * 지식인 황금질문(사장님 지시 2026-08-19) — 행 필터가 아니라 **질문 목록**이다.
     * 보드 전 행의 지식인 실측(최신 질문 + 조회수·답변수)에서 "조회는 많은데
     * 답변이 적은" 것만 모은다. 수요는 실측됐는데 공급이 빈 자리 — 황금 비율과
     * 같은 원리를 질문 단위로 적용한 것이다.
     */
    { id: 'kin-golden', label: '지식인 황금질문', hint: '조회는 많은데 답변이 적은 최신 질문 실측 — 답변이나 글로 선점할 자리' },
