...
export function finalizeStructuredContent(
  ...
  //   소제목 추출기가 "짧고 마침표 없는 줄"을 소제목으로 잡아 본문 일부에 인용구가
  //   씌워졌다(사용자 실측). 보정은 붙여넣기 직전 표현 계층으로 옮겼다
  //   — src/automation/richTextPaste.ts buildReadableParagraphs().
...
export function buildModeBasedPrompt(
  ...
  /**
   * [2026-09-15 사장님 요청] 요약표를 껐으면, 앞에서 붙은 [BRIEF-HEAD] 의 "요약 표" 지시를 취소한다.
   * 스키마에서 summaryTable 을 빼는 것만으로는 모델이 **도입부에 표를 직접 그릴** 수 있다 —
   * 프롬프트가 여전히 "도입부 바로 뒤 = 사실 요약 표" 라고 말하고 있기 때문이다. 둘 다 꺼야 실제로 안 나온다.
   *
   * [2026-09-16] 표가 도입부 뒤로 옮겨졌으므로 취소문이 가리키는 **규칙 이름도 같이 옮긴다.**
   * 없어진 이름("글 맨 앞 사실 요약 표")을 취소하라고 하면 모델이 짚을 대상을 못 찾는다.
   */
  if (source.includeSummaryTable === false) {
    systemPrompt = `${systemPrompt}

[표 설정 — 위 규칙보다 우선]
· 이 글에는 **요약표를 넣지 않는다.** 앞의 [BRIEF-HEAD] 중 "도입부 바로 뒤 = 사실 요약 표" 규칙은 적용하지 않는다.
