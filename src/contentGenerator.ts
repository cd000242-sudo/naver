...
import { buildSystemPromptFromHint, buildFullPrompt, loadShoppingPrompt, type PromptMode } from './promptLoader.js';
...
【글톤: ${customTone}】
${{
  friendly: '- 친한 친구가 카톡으로 수다 떠는 따뜻한 구어체',
  professional: '- 10년차 전문가의 냉철하고 논리적인 분석 칼럼체',
  casual: '- 인스타/트위터 감성의 산뜻하고 발랄한 말투, 격식 0%',
  formal: '- 아나운서 수준의 극도로 정중한 하십시오체',
  humorous: '- 재치 있는 비유와 설정→반전 구조의 엔터테이너 말투',
  community_fan: '- 네이트판/더쿠의 찐팬 실시간 호들갑 수다, 날것 그 자체',
  mom_cafe: '- 맘카페 살림 고수의 친절한 언니 말투, 텍스트 이모티콘(^^, ㅎㅎ) 활용',
  storyteller: '- 짧은 에세이/수필 내레이터, 시간축 서사+오감 묘사',
  expert_review: '- IT 매체 에디터의 체계적 평가+한줄 판정, 장단점 7:3',
  calm_info: '- 도서관 사서처럼 차분하고 신뢰감 있는 정보 전달체',
}[customTone] || '- 사용자 설정 글톤에 맞는 자연스러운 문체'}
...
【할루시네이션(거짓말) 방지】
- 원문(rawText)에 없는 구체적 수치(무게 g, 시간 h, 용량 mAh)를 절대 지어내지 마세요
- 확실하지 않은 정보는 "~라고 해요", "약~", "대략~" 등으로 완화 표현 사용
- 특정 브랜드를 근거 없이 비교하거나 비방하지 마세요
...
□ 순수 JSON만 출력했는가?`;
    ...
    if (shoppingPrompt) {
      ...
      // .prompt 파일 로드 실패 → 기존 인라인 폴백
      console.warn(`[PromptBuilder] ⚠️ .prompt 파일 로드 실패, 인라인 폴백 사용`);
      systemPromptResult += `
...
1. **Tone Sync (${toneStyle}):**
   - 이 글의 톤은 STYLE OVERRIDE(${toneStyle})에 맞춰 작성하세요.
   - 고정된 예시 문장을 복사하지 말고, 톤의 페르소나에 맞게 AI가 자유롭게 추론하여 작성하세요.
   - 같은 어미 2회 연속 금지. 문장 연결감을 유지하세요.
2. **Time Sync:**
...
`;
      console.log(`[PromptBuilder] ✅ 쇼핑커넥트 모드: 인라인 폴백 (Transcendence Mode) 적용`);
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🔍 [리뷰형 글 - 구매 전 제품 분석/가이드 지침]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

⚠️ 중요: 이 글은 **"사용후기"가 아닙니다!**
이 글은 **구매를 고민하는 독자가 구매 결정을 내릴 수 있도록 돕는 "제품 분석 가이드"**입니다.
"써봤는데", "사용해보니", "도착해서 열어보니" 같은 사후 경험 표현을 **절대 사용하지 마세요.**

📐 **글 관점 (필수)**
- 당신은 **제품/서비스 전문 에디터**이며 구매 전 정보를 제공하는 역할입니다.
...
4. **장점 심층 분석** → 경쟁 제품 대비 강점은 무엇인지 (구체적 수치/기능 비교)
5. **아쉬운 점 & 주의사항** → 구매 전 반드시 알아야 할 단점/한계 (균형 잡힌 시각)
6. **가격 & 가성비 판단** → 이 가격대에서 합리적인 선택인지, 대안은 없는지
7. **구매 가이드 / 최종 판단** → "결론적으로 이런 분이면 사세요 / 이런 분은 다른 걸 보세요"

🎯 **리뷰형 핵심 톤 & 매너**
- ❌ 금지: "직접 써보니", "사용해보니", "도착했을 때", "개봉하자마자", "2주 써봤는데"
- ❌ 금지: "재구매 의향 있어요", "다시 살 거예요" (사용후기 톤)
- ✅ 권장: "스펙을 살펴보면", "주목할 점은", "비교해보면", "이 가격대에서는", "구매 전 체크포인트"
- ✅ 권장: "~라는 평가가 많아요", "전문가들은 ~라고 분석해요"

📊 **분석적 표현 예시**
| ❌ 사용후기형 (금지) | ✅ 리뷰형 (권장) |
|---------------------|-----------------|
| 써봤는데 진짜 좋아요 | 주목할 만한 기능은 OO입니다 |
| 배송 빨랐어요 | 공식 스토어 기준 2~3일 내 배송 가능 |
| 가족들도 좋아해요 | 가족 구성원별로 활용도가 높은 제품 |
| 재구매 의향 있어요 | 이 가격대에서 가성비가 뛰어난 선택지 |
| 단점은 딱히 없어요 | 한 가지 체크할 점은 OO 부분 |

⚠️ **할루시네이션 방지**
- 원문(rawText)에 없는 스펙 수치(무게, 용량, 와트 등)를 절대 지어내지 마세요.
- 원문에 있는 정보만 활용하고, 없는 정보는 "공식 사양을 확인해보세요" 등으로 안내하세요.

🏆 **리뷰형 제목 생성 규칙 (필수)**
- ❌ 금지 키워드: "실사용", "솔직후기", "내돈내산", "찐후기", "리얼후기", "사용법" (이건 사용후기형 표현!)
- ✅ 권장 패턴: "[제품명] 구매 전 꼭 알아야 할 OO가지", "[제품명] 스펙 비교 총정리", "[제품명] 이 가격 합리적일까?"
- 제목에 제품명은 반드시 포함하되, 25~40자 이내로 작성
- "구매 가이드", "스펙 분석", "장단점 비교", "체크포인트" 등 분석적 키워드 사용
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
⚠️⚠️⚠️ [홈판 필수] 서브키워드 본문 밀도 규칙 ⚠️⚠️⚠️
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

네이버 AI는 제목뿐만 아니라 본문 전체에서 토픽을 분류합니다.
서브키워드가 본문에 충분히 녹아있지 않으면 토픽 매칭에 실패하여
홈피드에 절대 노출되지 않습니다.
...
📌 스크롤 트리거 문장 (3개 이상 의무):
본문 중간중간에 아래와 같은 스크롤유도 문장을 반드시 3개 이상 삽입하세요:
- "근데 여기서 반전이에요"
- "사실 이게 끝이 아니거든요"
- "이쯤 되면 궁금하실 겁니다"
- "여기서 중요한 건요"
