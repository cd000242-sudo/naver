...
export const AI_CONCLUSION_PHRASES = [
  '결론적으로', '정리하면', '정리해서 말하면', '정리하자면', '요약하면', '요약하자면', '종합하면',
...
const PC_PATTERNS = [
  /근데[^.]{0,30}?다음[^.]{0,40}?(중요|핵심|관건|진짜|결정적|까다로운|놓치|어려운|복잡)/,
  /다음[^.]{0,40}?(중요|핵심|관건|결정적|까다로운|놓치|어려운|복잡|진짜)/,
  /여기서\s*한\s*가지[^.]{0,40}?다음/,
  /놓치면\s*안\s*되[^.]{0,40}?다음/,
  /이제부터가?\s*(더\s*)?까다로운|이제부터|지금부터/,
  /다음\s*항목|다음에\s*다룰|다음에\s*설명|다음에\s*정리|다음에\s*나올/,
  /진짜\s*(중요한|헷갈리는|어려운)/,
  /절반은\s*끝/,
];
const PD_PATTERNS = [
