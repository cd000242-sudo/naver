...
describe('fact-brief-header — 주제를 가리지 않는 첫 화면', () => {
  ...
  it('요약 표와 날짜 앵커 둘만 담는다', () => {
    ...
    /*
     * [2026-09-01] "YYYY년 M월 D일 기준," 을 무조건 요구하는 형태를 박제하고 있었다.
     *
     * 모델은 오늘 날짜를 모른다 — 본문 프롬프트에 현재 날짜가 주입되는 지점이 없다
     * (contentGenerator 는 제목 프롬프트에만 연도를 붙인다). 자료에 기준일이 없으면
     * 그 자리를 채우는 방법은 추정뿐이고, 추정한 날짜는 그 자체로 날조다.
     *
     * 게다가 같은 조립물 앞쪽 seo/base.prompt F1 이 이미
     * "○월 ○일 기준 작성일 못박기 금지" 를 선언한다. 뒤에 붙는 이 파일이 위치로 이겨서,
     * 우리 금지를 우리 의무가 덮고 있었다.
     *
     * 위 주석의 판단("절차·비용 글에 오히려 더 맞는다 — 신청 조건·요금·기준일")은
     * 그대로 유효하다. 그 기준일이 자료에 있을 때의 이야기라는 조건만 명시한다.
     */
    expect(header).toContain('입력 자료에 시행일·개정일·기준일이 있으면');
    expect(header).toContain('자료에 날짜가 없으면 날짜 앵커를 쓰지 말고');
    /*
