...
describe('홈판 제목 길이 계약 (2026-08-26 사장님 실측)', () => {
  ...
  it('이슈픽 3공식에도 길이가 생겼다 — 미완성 서술은 짧아야 산다', () => {
    // [2026-09-17 실측] 기준을 글자수에서 폭(한글 1, 그 외 0.5)으로 통일하고,
    // 하한을 28 → 33 으로 올렸다. 구간별 lift: 28미만 0.69 / 28~33 0.97 / 33~38 1.20.
    // 상한은 후킹 장치 3개 미만일 때만 건다 — 길이 자체가 문제였던 적은 없다.
    expect(issueStory).toMatch(/33 이상/);
    expect(issueStory).toMatch(/장치를 3개 이상 담았으면 42를 넘겨도 된다/);
