...
describe('길이 계약이 점수보다 앞선다', () => {
  ...
  // [2026-09-17] 하한이 28 → 33 으로 올라가 기존 시료(폭 29.0)가 '짧음' 이 됐다.
  //   28~33 은 실측 0.97배로 이득이 없는 구간이라 계약 안이 아니다. 폭 34.0 으로 교체.
  const FIT = '전현무 카자흐스탄 즉흥 여행 조작설, 무편집 영상이 통째로 뒤집은 그날';
