  ...
  it('caches one immutable current-artifact identity and rejects each stale pin', () => {
    ...
    expect(current).toEqual({
      ...
      legacyBaselineSha256:
        ...
        // [2026-09-09] 사진 글 소제목을 문장형으로(imageNarrative/base.prompt M3 · 출력 스키마)로 재계산
        ...
        // [2026-09-11 2차] 본문 질문 하한을 신설해 재계산 — 6월 글(사장님이 "사람보다 사람"으로
        //   기억)과 9월 글을 가른 단일 최대 신호가 질문이었다. 편당 4.82개 → 0.78개(6.2배 감소),
        //   중반 60% 구간이 3.06 → 0.57 로 절대 손실 최대. 원인은 8개 카테고리 프롬프트에 깔린
        //   "자료에 답이 없는 질문은 세우지 않는다" 조건뿐이고 질문을 던지라는 지시가 없던 것.
