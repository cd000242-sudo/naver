...
describe('v1.4.77 — 비용 최적화 소스 불변식', () => {
  ...
  describe('OpenAI 재시도 — 429 누진 backoff가 끝까지 발동', () => {
    ...
    it('keeps structural retries bounded while agent drafts default to one paid call', () => {
      ...
      expect(content).toMatch(/const\s+sameEngineReliabilityMinAttempts\s*=\s*isV3Prompt\s*\|\|\s*isAgentProvider\s*\?\s*0\s*:\s*readNonNegativeIntegerEnv\('CONTENT_SAME_ENGINE_MIN_ATTEMPTS',\s*0\)/);
