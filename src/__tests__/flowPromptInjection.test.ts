...
describe('softenSensitiveScene — 금융/민감 객체 장면 선제 완화', () => {
    it('뱅킹앱·금융서류 객체 감지 시 일반 장면으로 치환 (시점 보존)', () => {
        const p = 'CRITICAL FRAMING: low-angle.\n\nProfessional photo of a Korean adult reviewing financial documents and a banking app on a tablet.';
        const out = softenSensitiveScene(p);
        expect(out.softened).toBe(true);
        expect(out.prompt).toContain('CRITICAL FRAMING: low-angle');     // 시점 힌트 보존
        expect(out.prompt).not.toMatch(/banking app|financial documents/i); // 트리거 제거
        expect(out.prompt.toLowerCase()).toContain('workspace');         // 일반 장면
    });

    it('통장/카드명세서 등 한글 객체도 감지', () => {
        expect(softenSensitiveScene('적금 통장을 들고 있는 사람').softened).toBe(true);
        expect(softenSensitiveScene('신용카드 명세서를 보는 장면, 카드 명세서').softened).toBe(true);
    });

    it('객체 없는 일반/추상 장면은 완화하지 않음 (오탐 방지)', () => {
