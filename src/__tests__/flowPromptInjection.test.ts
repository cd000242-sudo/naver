...
import {
    ...
    simplifyFlowPrompt,
    softenSensitiveScene,
    HEADING_VARIATION_HINT_COUNT,
} from '../image/flowPromptInjection';

describe('simplifyFlowPrompt — 퀄리티 보존 우회 (장면 유지, 트리거만 제거)', () => {
    const sample =
        '# Subject (must reflect this heading): 청년내일저축계좌 가입 소득 33만원 재산 기준\n\n' +
        'CRITICAL FRAMING: wide-angle.\n\nA young person reviewing documents at a bright desk, 2026 budget, 50% match scene.';

    it('level 0은 원본 유지', () => {
        expect(simplifyFlowPrompt(sample, 0)).toBe(sample);
        expect(simplifyFlowPrompt('', 2)).toBe('');
    });

    it('level 1은 텍스트 변경 없음 (새 프로젝트 재시도용 — 풀 퀄리티)', () => {
        expect(simplifyFlowPrompt(sample, 1)).toBe(sample);
    });

    it('level 2는 한글 Subject 줄만 제거하고 영어 장면은 보존 (풀 퀄리티)', () => {
        const out = simplifyFlowPrompt(sample, 2);
        expect(out).not.toMatch(/# Subject/);
        expect(out).not.toContain('청년내일저축계좌');           // 트리거 제거
        expect(out).toContain('A young person reviewing documents'); // 영어 장면 보존
        expect(out).toContain('wide-angle');                       // 시점 힌트 보존
        expect(out).toMatch(/2026|50%/);                           // level 2는 숫자 유지
    });

    it('level 3은 Subject 제거 + 숫자/금액/퍼센트 중화, 장면은 유지', () => {
        const out = simplifyFlowPrompt(sample, 3);
        expect(out).not.toContain('청년내일저축계좌');
        expect(out).not.toMatch(/\d{2,}/);                         // 33/2026/50 등 제거
        expect(out).not.toMatch(/%/);
        expect(out.toLowerCase()).toContain('young person reviewing documents'); // 장면 여전히 유지
    });

    it('과도 제거로 비어도 원본을 유지 (빈 문자열 방지)', () => {
        const onlySubject = '# Subject (must reflect this heading): 정책\n\n';
        expect(simplifyFlowPrompt(onlySubject, 2)).toBe(onlySubject); // 전부 제거되면 원본 반환
    });
});

describe('softenSensitiveScene — 금융/민감 객체만 치환, 장면(연관성)은 보존', () => {
    it('트리거 객체만 안전 등가물로 치환하고 인물·상황·구도는 유지', () => {
        const p = 'CRITICAL FRAMING: low-angle.\n\nProfessional photo of a Korean adult reviewing financial documents and a banking app on a tablet, modern desk, contemplative mood.';
        const out = softenSensitiveScene(p);
        expect(out.softened).toBe(true);
        expect(out.prompt).not.toMatch(/banking app|financial documents/i);   // 트리거 제거
        // 장면 연관성 보존 — 인물/상황/구도/분위기 그대로
        expect(out.prompt).toContain('CRITICAL FRAMING: low-angle');
        expect(out.prompt).toContain('Professional photo of a Korean adult reviewing');
        expect(out.prompt).toContain('modern desk, contemplative mood');
        expect(out.prompt.toLowerCase()).toMatch(/notebook|planning notes/);   // 안전 등가물로 치환됨
    });

    it('통장/카드명세서 등 한글 객체도 치환', () => {
        const a = softenSensitiveScene('적금 통장을 들고 미소짓는 사람, 밝은 거실');
        expect(a.softened).toBe(true);
        expect(a.prompt).not.toContain('통장');
        expect(a.prompt).toContain('미소짓는 사람, 밝은 거실'); // 장면 보존
        expect(softenSensitiveScene('카드 명세서를 보는 장면').softened).toBe(true);
    });

    it('객체 없는 일반/추상 장면은 그대로 (오탐 방지)', () => {
        expect(softenSensitiveScene('a happy person celebrating financial freedom on a sunny beach').softened).toBe(false);
        expect(softenSensitiveScene('가을 캠핑장에서 커피를 마시는 사람').softened).toBe(false);
        expect(softenSensitiveScene('').softened).toBe(false);
    });
});
