...
describe('보완 문구는 사람이 쓰는 말과 할 일을 담는다', () => {
  ...
  it('단위가 지표에 맞는다 — 길이는 자, 점수는 점', () => {
    ...
    expect(joined).toMatch(/(?:사람이 쓴 느낌|글의 방향) \d+점/u);
  });

  /*
   * [2026-09-10] 안전성은 비교 축에서 뺐다. safetyEval 의 Fidelity 는 rawText(우리가 모은
   * 자료)가 있어야 매겨지는데 남의 글에는 그것이 없어 상위 문서가 항상 불리했다.
   * 실측 리포트에 "표현 안전 우리 100점 vs 상위 48점 → 상위 글보다 좋습니다" 가 떴고,
   * 그 48점짜리 글이 1위였다.
   */
  it('안전성은 비교하지 않는다 — 상위 문서가 구조적으로 불리한 축이다', () => {
    expect(fixes().join(' | ')).not.toMatch(/표현 안전/u);
  });
