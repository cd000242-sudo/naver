...
describe('회귀방지 net: 사람다움/AI탐지 분별력', () => {
  ...
  describe('evaluateHumanlike', () => {
    ...
    // 저장본 실측 회귀 — 사람 같다고 평가된 표본 17편 중 10편(59%)이 말끝 때문에
    // 평균 7.1점 감점을 맞고 있었다. 말끝 밀도가 실측 상한(14.6/1k) 이하면 감점 0 이어야 한다.
    it('사람 말투 밀도가 실측 범위 안이면 입말 감점을 매기지 않는다', () => {
      // 6월 표본 프로필 재현 — 거든요 5 · 잖아요 3 · 더라고요 2 · 진짜 3, 본문 1,200자대.
      // 구 규칙에서는 반복 5회로 감점이 걸렸고, 실측상 사람 같다고 평가된 글의 전형이다.
      const natural = [
        '신청 기간이 헷갈리시죠. 저도 처음엔 마감일을 잘못 봤거든요.',
        '공고를 다시 열어보니 접수 시작일과 서류 마감일이 따로 적혀 있더라고요.',
        '두 날짜를 같은 칸에 적어두면 마감 당일에 헷갈리거든요. 달력에 나눠 적는 편이 낫습니다.',
        '서류는 미리 스캔해두면 급하게 찾을 일이 없잖아요.',
        '지자체마다 요구 서류가 조금씩 다르니 공고문 원문을 한 번 더 확인하세요.',
        '접수처 전화번호는 공고문 하단에 있고, 점심시간 직후에는 통화가 몰립니다.',
        '준비물은 신분증과 통장 사본 정도면 대부분 충분했습니다.',
        '다만 세대 분리가 된 경우에는 서류가 한 장 더 붙거든요.',
        '이 부분이 진짜 자주 놓치는 지점입니다.',
        '온라인 접수는 공동인증서가 있어야 하잖아요. 미리 유효기간을 확인해두면 좋습니다.',
        '작년에는 접수 첫날 오전에 시스템이 느려졌더라고요.',
        '그래서 첫날 오전보다는 이틀째 오후가 오히려 수월했습니다.',
        '지원 대상 기준은 소득이 아니라 거주 기간으로 잡히는 경우도 있거든요.',
        '공고문에 적힌 기준일이 진짜 중요합니다. 기준일 하루 차이로 대상이 갈립니다.',
        '서류를 다 모으고도 접수처를 잘못 찾아가는 경우가 진짜 많잖아요.',
        '방문 접수와 온라인 접수의 마감 시각이 다른 곳도 있으니 그것도 확인하세요.',
        '마지막으로 접수증은 사진으로 남겨두는 편이 안전합니다.',
        '접수 이후 심사 결과는 보통 2주에서 4주 사이에 개별 안내로 옵니다.',
        '문자로만 오는 지역도 있어서 스팸 차단 설정을 미리 풀어두는 편이 좋습니다.',
        '결과가 늦어질 때는 접수번호를 들고 문의하면 조회가 빠릅니다.',
        '탈락 통보를 받아도 다음 회차 접수가 남아 있는 경우가 대부분입니다.',
        '회차별 예산이 다르게 잡히기 때문에 초회차 경쟁이 늘 더 치열한 것도 아닙니다.',
        '같은 서류를 재사용할 수 있는지 여부는 접수처에 먼저 확인해두면 시간이 절약됩니다.',
      ].join(' ');
      const score = evaluateHumanlike({ body: natural, mode: 'homefeed' });
      expect(score.details.toneMarkerPer1000).toBeLessThanOrEqual(18);
      expect(score.details.conversationalCrutchPenalty).toBe(-0);
    });

    it('말끝 도배는 여전히 감점한다', () => {
      const flood = '진짜 그렇거든요. 진짜 맞잖아요. 진짜 그러더라고요. 거든요 잖아요 더라고요.';
      const score = evaluateHumanlike({ body: flood, mode: 'homefeed' });
      expect(score.details.toneMarkerPer1000).toBeGreaterThan(18);
      expect(score.details.conversationalCrutchPenalty).toBeLessThan(0);
    });
  });
