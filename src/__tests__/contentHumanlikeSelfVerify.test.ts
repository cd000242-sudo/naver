...
const seo = (body: string): EvaluationInput => ({ body, mode: 'seo', firstPartyEvidenceAvailable: false });
const seoWithExperience = (body: string): EvaluationInput => ({
  body,
  mode: 'seo',
...
describe('자체검증: evaluateHumanlike 합성 글 — 사람글이 AI글보다 사람답다', () => {
  it('사람글 점수 ≥ 60, AI글 점수 ≤ 40, 격차 ≥ 25', () => {
    const human = evaluateHumanlike(seoWithExperience(HUMAN_ARTICLE));
  ...
  it('AI글은 클리셰 부재 점수 0 + 사람글은 만점(15)', () => {
    ...
    expect(evaluateHumanlike(seoWithExperience(HUMAN_ARTICLE)).details.noAiCliche).toBe(15);
  });

  it('경험 근거가 있는 글과 체험을 꾸미지 않은 글 모두 화자 정합성 만점', () => {
    const human = evaluateHumanlike(seoWithExperience(HUMAN_ARTICLE));
    const ai = evaluateHumanlike(seo(AI_ARTICLE));
    expect(human.details.directExperience).toBe(15);
    expect(ai.details.directExperience).toBe(15);
  });
});
...
describe('자체검증: 7개 변수 각각 단독 응답 (고 vs 저)', () => {
  ...
  it('3) 전환 마커 — 없거나 절제하면 높고, 반복하면 낮다', () => {
    const none = '이것은 좋은 제품입니다 그리고 성능이 우수합니다 또한 가격도 적당합니다';
    const many = '솔직히 처음엔 별로였어요. 근데 막상 써보니 괜찮더라고요. 알고보니 설정 문제였어요. 의외로 만족했죠.';
    expect(evaluateHumanlike(seo(none)).details.selfCorrection).toBe(12);
    expect(evaluateHumanlike(seo(many)).details.selfCorrection).toBeLessThan(12);
  });
  ...
  it('5) 직접 경험 신호 — 없으면 낮고, 있으면 높다', () => {
    ...
    expect(evaluateHumanlike({ ...seo(noExp), firstPartyEvidenceAvailable: true }).details.directExperience)
      .toBeLessThan(evaluateHumanlike({ ...seo(withExp), firstPartyEvidenceAvailable: true, groundingText: withExp }).details.directExperience);
  });
});
...
describe('자체검증: 엔드투엔드 게이트 — 시스템이 스스로 human/AI를 판정', () => {
  it('게이트 humanlike 하위점수가 사람글 > AI글, finalScore도 사람글 ≥ AI글', () => {
    const human = evaluate(seoWithExperience(HUMAN_ARTICLE));
...
// ─────────────────────────────────────────────────────────────
// 홈판 모드 자체검증: 감정어 개수보다 정보 가치와 절제된 리듬을 우선
// ─────────────────────────────────────────────────────────────
...
const USEFUL_HOMEFEED = [
  '신청 버튼부터 찾으면 중간에 다시 돌아오기 쉽습니다.',
  '먼저 볼 건 대상 조건과 준비 서류예요.',
  '조건이 맞는지 표시해 두면 신청할지 판단하기 쉬워집니다.',
  '- 대상 조건',
  '- 준비 서류',
  '- 공식 신청처',
].join('\n\n');

describe('자체검증: 홈판 — 가치 우위 + 자연스러운 감정 절제', () => {
  it('독자 가치가 있는 홈판 글이 감정어를 연출한 글보다 높다', () => {
    expect(evaluateHomefeed({ ...hf(USEFUL_HOMEFEED), title: '정부지원 신청 전에 서류부터 보면 덜 헷갈려요', primaryKeyword: '정부지원 신청' }).score)
      .toBeGreaterThan(evaluateHomefeed({ ...hf(EMOTIONAL_HOMEFEED), firstPartyEvidenceAvailable: true, groundingText: EMOTIONAL_HOMEFEED }).score);
  });
...
describe('자체검증: isPlatitudeHardBlockEnabled — 옵트인 차단 플래그', () => {
  const KEY = 'CONTENT_HARDBLOCK_ON_PLATITUDE';
  afterEach(() => { delete process.env[KEY]; });

  it('미설정 시 기본 OFF (자동화 안전 — 발행 안 멈춤)', () => {
    delete process.env[KEY];
    expect(isPlatitudeHardBlockEnabled()).toBe(false);
  });

  it.each(['1', 'true', 'on', 'TRUE', ' On '])('truthy 값 %s 에서 ON', (val) => {
    process.env[KEY] = val;
    expect(isPlatitudeHardBlockEnabled()).toBe(true);
  });

  it.each(['0', 'false', 'off', '', 'yes', 'enable'])('그 외 값 %s 은 OFF', (val) => {
    process.env[KEY] = val;
    expect(isPlatitudeHardBlockEnabled()).toBe(false);
  });
});
