...
/**
 * 에이전트 모드 — 사용자 본인 PC의 codex/claude/gemini CLI를 본인 구독으로 호출하는 글생성 엔진.
 ...
 *   'agent-gemini' → Google 구독(gemini CLI, 사용자 표기 "Antigravity").
...
export const AGENT_TEXT_PROVIDERS = ['agent-codex', 'agent-claude', 'agent-gemini'] as const;
...
export function isAgentTextProvider(value: unknown): value is AgentTextProvider {
  return value === 'agent-codex' || value === 'agent-claude' || value === 'agent-gemini';
}

/** 에이전트 provider 값 → agentCli 서비스 provider('codex'|'claude'|'gemini'). */
export function agentTextProviderToCli(value: AgentTextProvider): 'codex' | 'claude' | 'gemini' {
  if (value === 'agent-codex') return 'codex';
  if (value === 'agent-gemini') return 'gemini';
  return 'claude';
}
...
export function resolveTextModelProfile(value: unknown): TextModelProfile {
  ...
  if (isAgentTextProvider(selector)) {
    const agentDisplayName = selector === 'agent-codex'
      ? 'Codex Agent'
      : selector === 'agent-gemini'
        ? 'Gemini Agent'
        : 'Claude Agent';
    return profile(selector, 'agent', 'premium', selector, agentDisplayName);
  }
