...
- 이때 제목은 근거 없이 답할 수 없는 약속(정체 공개·숫자 충격·내부자 정보·확인 방법)을 걸지 않는다. 답을 못 줄 제목이면 그 제목을 버린다.`;
  // [2026-07-29] 라이브 실측(224357080956): "현재 자료에는 ~가 제시되지 않았으므로
  // 안내할 수 없어요" 류 근거 메타 서술이 본문에 그대로 노출돼 독자가 이탈했다.
  // 근거 게이트는 내부 판단 규칙이지 독자용 문장이 아니다 — 전 모드 공통 봉인.
  const metaLeakRule = `- 자료의 존재·부족·범위를 독자에게 말하지 않는다. "제공된 문구", "현재 자료에는", "원문에 특정되지 않", "확인되지 않았습니다", "이 숫자만으로는" 같은 근거 메타 서술을 본문에 쓰지 않는다. 모르는 항목은 조용히 생략하거나, 필요하면 "공식 안내에서 확인하는 게 정확합니다" 한 번으로만 처리한다.`;
