...
/**
 * [2026-07-29 신설, 2026-07-30 분리 export] 근거 메타 노출 금지 — 라이브 실측
 * (224357080956): "현재 자료에는 ~가 제시되지 않았으므로 안내할 수 없어요" 류
 * 근거 메타 서술이 본문에 노출돼 독자가 이탈했다. 근거 게이트는 내부 판단
 * 규칙이지 독자용 문장이 아니다. FINAL CONTRACT(일반 모드)와 쇼핑커넥트
 * finalContract 양쪽에서 공유한다.
 */
export function buildEvidenceMetaLeakRule(): string {
  return `- 자료의 존재·부족·범위를 독자에게 말하지 않는다. "제공된 문구", "현재 자료에는", "원문에 특정되지 않", "확인되지 않았습니다", "이 숫자만으로는" 같은 근거 메타 서술을 본문에 쓰지 않는다. 모르는 항목은 조용히 생략하거나, 필요하면 "공식 안내에서 확인하는 게 정확합니다" 한 번으로만 처리한다.
- [2026-09-03 라이브 224399815476] 자료 목록 자체를 서술하지 않는다: "검색 결과에는 2025년 9월 17일에 작성된 글도 남아 있습니다", "~글에는 ~이 등장합니다" 처럼 자료의 작성일·개수·출처를 나열하는 문단은 통째로 금지. 자료는 읽고 소화해서 사실만 쓴다.
- 남의 글을 중계하지 않는다: "후기에서는", "~라는 후기도 있어", "현장 후기의 관찰이므로", "~라고 소개됐습니다". 사실은 사실로 말하고, 변동 가능한 것은 "출발 전 확인" 한 줄로 묶는다.
- "~인지가 기준이에요", "~인지를 보면 됩니다" 같은 추상 판정문으로 문단을 닫지 않는다. 판단은 구체 행동으로 쓴다(예: "이동이 1시간 넘으면 당일치기 대신 1박으로").`;
}
...
- 이때 제목은 근거 없이 답할 수 없는 약속(정체 공개·숫자 충격·내부자 정보·확인 방법)을 걸지 않는다. 답을 못 줄 제목이면 그 제목을 버린다.`;
  ...
  const homefeedPayoffRule = options?.usesIssueStorySkeleton
    ...
    // [2026-09-17 실측 보정] 홈판 1,283편의 답 위치는 제목 유형에 따라 갈렸다.
    //   정체를 감춘 제목 → 본문 8~20%(중앙값 12%) / 대상이 드러난 제목 → 10~36%(중앙값 20%)
    // 전자는 도입 3~5줄과 사실상 같지만, 후자까지 3~5문장으로 묶으면 홈판에 못 간 글의
    // 패턴(앞에서 다 풀어버림)이 된다. homefeed/base.prompt 의 TITLE PAYOFF 와 같은 기준이다.
    : `- 제목이 던진 질문·숫자·정체·방법은 본문에서 직접 답한다. 제목에서 정체를 감췄으면 도입부 첫 3~5문장 안에서 답하고, 대상이 이미 드러나 있으면 본문 앞 10~36% 구간 안에서 답한다. 확인 절차 안내로 답을 대체하지 않는다.`;
...
- 제목이 던진 질문·숫자·조건·방법은 도입부 첫 3~5문장 안에서 직접 답한다. 근거로 답할 수 없는 약속은 제목에서 뺀다.
...
${groundingRule}
${metaLeakRule}
- 숫자·날짜·인용·명단은 팩트 규율 6(WRITER GROUNDING 정본)을 따른다. 구체성이 필요하면 실제 조건, 대상, 순서, 예외를 쓴다.
