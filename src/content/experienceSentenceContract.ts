 ...
 *
 * [2026-09-01] `가\s*보` 가 "성에'가 보'이면" 에 걸려 정보 문장을 경험으로 몰았다.
 * "갔다 보다" 만 잡도록 뒤에 오는 어미(아 · 았)까지 요구한다.
 * 앞서 "신고"(申告) 오탐과 같은 종류다 — 짧은 조각은 조사에 걸린다.
 *
 * human-writing-anti-pattern 의 분류를 그대로 따른다: 판단 · 조사 · 정리 · 권유 · 전달은
 * 체험이 아니므로 검사 대상이 아니다. 직접 쓰고 · 가고 · 먹고 · 신어본 일만 본다.
 */
const FIRSTHAND = /(?:써\s*보|써봤|사용해\s*보|신어\s*보|신어봤|먹어\s*보|가\s*보[아았]|가봤|입어\s*보|발라\s*보|들어\s*보[니았아]|겪어\s*보|겪었|해\s*봤|해보니|써보니|살아\s*보[니았아]|지내\s*보[니았아]|달\s*(?:살아|지내|써)|개월\s*(?:살아|지내|써)|우리\s*집(?!\s*(?:근처|옆|앞|뒤|주변|앞쪽|근방)))/u;
