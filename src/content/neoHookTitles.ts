...
★ 패턴 이름을 먼저 고르고 글감을 끼워 맞추지 마라. 무엇을 다루는 상황인지를
  먼저 정하고, 그 상황이 드러나게 제목을 쓴다. 길이·장치·약속(payoff) 규칙은 base [TITLE] 정본을 따른다 —
  아래는 홈판(${cat}) 고유 제약(블랙리스트·반복 방지)만이다.

■ 구체성 재료 (상황을 쓸 때 실제로 있으면 살려라 — 없으면 만들지 말 것):
  장면(지하철·퇴근길·마트·약국) · 감각(관찰 가능한 대상일 때만) · 구체 시공(새벽·주말·N시) · 갈리는 지점(자료 근거가 있을 때만)
  ⛔ "이거 저만 그래요?", "다들 어떠세요?" 같은 빈 질문은 쓰지 않는다 — 독자는 답을 보러 왔다.
...
■ 반복 방지: 같은 블로거의 직전 5개 글 제목과 절반 이상 같은 단어 공유 금지. "저도 X 했는데 이렇게 됐어요" 패턴도 5번에 1번 이하로만.

■ 자가검토(고유 항목만): 블랙리스트 단어 0개인가? 직전 제목들과 다른 상황을 다루는가?
  이 훅이 약속한 정보(질문·숫자·정체·방법)를 근거 자료로 도입부에서 직접 답할 수 있는가? (답을 못 주는 훅은 답 가능한 훅으로 교체)
════════════════════════════════════════════════════════════
`;
}
