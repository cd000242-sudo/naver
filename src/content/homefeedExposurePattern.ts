...
3. 모바일 문단은 2~3문장(화면 2~3줄)으로 묶고 문단 사이는 빈 줄로 띄운다. 문장 하나를 문단 하나로 잘게 끊지 않는다. 다만 완결된 생각을 글자 수에 맞추려고 자르지 않는다.
3-1. 문단을 명사형으로 끊어도 된다. 실측 기준 홈판 문단의 42%가 마침표 없이 명사로 끝난다("…시선을 사로잡은 이유", "…달라진 기준"). 다만 명사형만 이어 붙여 판단 없는 나열이 되지 않게 한다.
