 ...
 */
// [2026-09-03 실측] "1일~1.5일 또는 2일로 묶을지" 의 기간 표기가 날짜로 잡혔다 — 물결·소수점·'로/정도/일정/코스' 문맥은 제외.
const DAY_WITHOUT_MONTH_RE = /(?<!\d\s?월\s?)(?<![\d.])(?<![~〜-]\s?)(\d{1,2})\s?일(?!\s?간|째|\s?동안|용|치|\s?로|\s?정도|\s?이상|\s?이내|\s?짜리|\s?[~〜-]|\s?또는|\s?일정|\s?코스)/g;
