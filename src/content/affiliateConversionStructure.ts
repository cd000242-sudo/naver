...
${source?.aiExperienceGeneration === true && !hasFirstPartyMemo(source) ? '4. 반복 장면 — 내가 쓰면서 여러 번 겪은 결과를 보여준다. 평점·리뷰 수 같은 숫자는 입력 자료에 실제로 있을 때만 쓰고, 없으면 쓰지 않는다.' : `4. 사회적 증거 — 구매자 후기에서 반복 확인된 결과를 보여준다. 평점·리뷰 수 같은 숫자는 입력 자료에 실제로 있을 때만 쓰고, 없으면 숫자 없이 후기 내용으로만. (Authoritativeness: 출처는 구매자 후기로 귀속)`}
${storyRule}
