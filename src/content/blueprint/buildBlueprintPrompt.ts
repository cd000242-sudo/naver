...
export interface BlueprintPromptInput {
  ...
  /**
   * [2026-09-05] User-confirmed title (keyword-as-title / manual override). When set, the angle
   * must be the question THIS title promises to answer — the model must not invent a new angle
   * from the keyword, or the body ends up answering a different question than the locked title.
   */
...
export function buildBlueprintPrompt(input: BlueprintPromptInput): string {
  ...
  const subject = confirmedTitle
    ? `확정 제목 "${confirmedTitle}"`
    : `키워드 "${keyword}"`;
  const angleRule = confirmedTitle
    ? `- angle: 확정 제목이 독자에게 답하겠다고 약속한 질문 하나(${L.angleMaxChars}자 이내). 제목과 다른 각도를 새로 만들지 않는다.`
    : `- angle: 이 글이 답할 질문 하나(${L.angleMaxChars}자 이내). 키워드를 검색한 사람이 실제로 묻는 것.`;
  const offTopicRule = confirmedTitle
    ? `- offTopic: 자료에 있지만 확정 제목의 질문에 답하지 않아 본문에서 빼야 할 주제를 최대 ${L.offTopicMax}개(각 한 줄). 관련 단어가 같아도 질문에 답하지 않으면 여기에 적는다. 없으면 빈 배열.`
    : `- offTopic: 자료에 있지만 이 키워드의 질문과 무관해 본문에서 빼야 할 주제를 최대 ${L.offTopicMax}개(각 한 줄). 없으면 빈 배열.`;
  return [
    `아래 [자료]를 읽고, ${subject} 로 쓸 블로그 글의 설계도를 JSON 하나로만 출력한다. 설명·마크다운·코드펜스 금지.`,
    '',
    '[규칙]',
    angleRule,
