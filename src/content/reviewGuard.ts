...
export interface ReviewGuardContext {
  ...
  hasSpec: boolean;
  hasPrice: boolean;
  /** [2026-09-03 사장님] AI 경험 옵트인 — 작성자 1인칭 체험 서술 허용(과거형 포함), 숫자 기간·구매·가족 주장은 금지 유지 */
...
이 글은 작성자 1인칭 체험으로 쓴다. 위 가드의 "체험 서술 금지"는 이 옵트인이 대체한다 — 확인된 스펙·구성에서
도출한 사용 장면을 내가 겪은 것처럼 쓴다("써보니", "저는", "쓰다 보니", 과거형·현재형 모두). 장면 하나는 사실 하나에서 나온다.
"후기에서는", "사용자들은" 같은 남의 경험 중계 문장은 쓰지 않는다.
그대로 금지: 숫자 기간("2주", "한 달"), 구매 시점, 내돈내산, 가족 반응, 다른 제품과의 사용 비교, 자료에 없는 수치·효과.`
    : '';
...
리뷰 데이터가 없으므로 제목의 **후기/리뷰/사용기/체험기 표기 자체도 금지**입니다.
...
데이터가 없으면 해당 섹션을 **통째로 생략**하고 억지로 채우지 마세요.
  ...
  ✓ 가격 포지셔닝 — 제공된 가격만 인용. 타사 비교 금지
      예: "수집 당시 판매 페이지 표시값은 [입력된 가격]이었고, 최신 가격과 옵션은 결제 전에 확인해보세요"
  ✓ 페르소나 매칭 CTA — "이런 분께 맞을 것 같아요" 형식
  ✓ 검증 필요 정보 이관 — 의사결정에 중요한 미확인 항목만 한 문단에서 1회 안내
...
[최종 자가검수 (내부 검증 전용 — 본문에 출력 금지)]
⛔ 이 체크리스트는 사일런트 검증이다. "솔직하게 자체비평하겠습니다",
   "자가검수 결과", "체크리스트로 확인하면" 같은 메타 문구를 본문/제목/도입/
   결론 어디에도 적지 마라. 검증 결과는 본문에 반영만 하고 검증 과정은 출력하지 않는다.
□ **제목에** "2주/한 달/1개월/100일/써보/실사용/직접 써본/후기/리뷰/사용기/체험기" 등 근거 없는 체험 표현이 0개인가?
...
□ "확인/상세 페이지/단정하기 어렵다" 계열은 가격 안내와 마지막 CTA를 합쳐 최대 2회인가?
□ 제품을 후보에 둘 이유와 가장 잘 맞는 사람이 구체적으로 드러나는가?
