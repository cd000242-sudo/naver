...
// [2026-08-12] 스토킹·협박 추가. 재판 진행 중 사건 글에서 위험명사가 없어 가드가
//   아예 발동하지 못했다(실측: 황정민 스토킹 재판 글에서 단정 5건 전부 미탐지).
const CRIME_NOUNS = ['학폭', '마약', '탈세', '음주운전', '폭행', '도박', '성추문', '성접대', '성 접대', '성상납', '성 상납', '갑질', '횡령', '성범죄', '불법촬영', '성매매', '주가조작', '시세조작', '데이트폭력', '몰카', '스토킹', '협박'];
...
//
// [2026-08-12] 법정 증명어(증명·입증·방증)와 비과거 활용형 추가.
//   연예 가십 어휘(구속·시인·자백)는 촘촘한데 재판 문맥에서 가장 위험한
//   "법정에서 증명됐다"가 통째로 비어 있었다. 선고 전 유죄 단정이라 리스크가 가장 크다.
//   '증명서'는 뒤가 서라서 매칭되지 않는다(증명됐/증명된/증명되만 본다).
const ASSERTION_RE = /(?:드러났다|드러난|드러나(?:고|면서)|밝혀졌다|밝혀진|밝혀져|밝혀지(?:고|면서|며)|확정됐다|확정된|사실로\s*확인|확인됐다|확인된|들통|판명(?:됐다|난|되)|증명(?:됐|된|되)|입증(?:됐|된|되)|방증|저질렀|전해졌다|전해진|알려졌다|알려진|포착(?:됐다|된)|인정(?:했다|됐다|된)|시인|자백|실토|적발|입건|구속|기소|발각|폭로(?:됐다|된)|한\s*것으로\s*(?:확인|드러|나타|밝혀|전해|알려))/;
...
[절대 금지 — 미확인 단정]
실존 인물의 (1) 범죄·비위(학폭·마약·탈세·음주운전·사기·폭행·성범죄·스토킹·협박 등) (2) 사생활(열애·결별·이혼·임신·불륜 등)
의혹을 "사실"로 단정하지 마세요. "드러났다/밝혀졌다/전해졌다/구속됐다/시인했다" 같은 단정·전언 종결 금지.

[재판 진행 중이면 특히 — 선고 전에 결론을 앞질러 쓰지 마세요]
구형·기소·약식명령·공판은 아직 판단 전 단계입니다. "법정에서 증명됐다 / 입증됐다 / 강력한 방증 /
실체가 드러났다"처럼 유죄를 확정한 표현을 쓰지 마세요. 대신 "공판에서 이런 정황과 자료가 제시됐다",
"검찰은 ~를 구형했다"처럼 **누가 무엇을 주장·요청했는지**로 쓰면 후킹은 그대로면서 정확해집니다.
구형액과 선고액, 형사와 민사, 청구액과 인정액을 섞어 쓰지 마세요.
