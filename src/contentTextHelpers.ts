...
export function stripInternalMarkers(s: string): string {
  ...
  return s
    ...
    // [2026-08-04] LLM이 본문 안에 이미지 지시문을 쓰는 누출 차단 — "[이미지 설명] ..." 류.
    // 마커로 시작하는 줄은 뒤따르는 지시문 텍스트까지 통째로 지운다
    // (마커만 지우면 "밝은 경기장 전경" 같은 지시문이 본문에 남는다).
    .replace(/^[ \t]*[[(【][^\])】\n]{0,20}(?:이미지|사진|삽화|그림)\s*(?:설명|프롬프트|묘사|제안|삽입)[^\])】\n]{0,60}[\])】][^\n]*$/gm, '')
    // 문장 중간에 낀 브래킷형 이미지 마커는 마커만 제거.
    .replace(/\s*[[(【]\s*(?:이미지|사진|삽화|그림)\s*(?:설명|프롬프트|묘사|제안|삽입)[^\])】\n]{0,60}[\])】]/g, '')
    // 작성일 못박기 제거 — codex 등 에이전트가 환경 날짜로 "2026-06-21 기준"을 박는 경우 strip.
    // 전체 날짜(연-월-일)+"기준"만 제거. 연도만 있는 "2026년 기준"이나 자료 날짜는 보존.
    // [2026-09-22 live 셀토스] 문장 첫머리의 스탬프만 지운다. 문장 중간의 자료 시점
    // ("기아 공식 가격표의 2026년 9월 1일 기준으로 …")까지 지워 "가격표의 이고"가 남았다.
    .replace(/(^|[\n.!?。]\s*)\d{4}\s*[-.년]\s*\d{1,2}\s*[-.월]\s*\d{1,2}\s*일?\s*기준[으로]*[,.\s]*/g, '$1')
    // 줄 삭제로 생긴 3연속 빈 줄 정리.
    .replace(/\n{3,}/g, '\n\n');
}
