...
export function buildSystemPrompt(
  ...
  if (!basePrompt) {
    ...
    basePrompt = getFallbackPrompt(mode);
  }
  ...
  // 2.6 상황-공감 깊이 보강 — seo/mate 도입부 보강.
  //     [2026-09-22 P1 홈판 병합] 홈판은 [GAMMA-7] 도입부 정본이 SD-1~3(상황 깊이·가치 명시·직답↔긴장)을
  //     이미 담고 있어 같은 규칙을 두 번 주지 않는다 — 홈판에서는 주입하지 않는다.
  if (mode !== 'homefeed') {
...
${claimDiscipline}`;
  ...
  //   검색어와 겹칠 면이 그만큼 줄어든다.
  // [2026-09-01] 업체 모드가 해시태그 전략에서 빠져 있었다. 출력 스키마에는 필드가 있는데
  //   (contentJsonPromptFormat 의 "해시태그1","해시태그2","해시태그3") 만드는 법을
  //   아무도 알려주지 않아 예시 3개만 보고 아무거나 달았다.
  //   업체 글이야말로 "강남임플란트 + 강남임플란트비용" 같은 지역·업종 조합 롱테일이
  //   필요한 자리다 — 그 자리를 비워 두고 있었다.
...
${anglesOverlay}`;
  ...
  if (mode === 'seo' || mode === 'homefeed' || mode === 'mate' || mode === 'affiliate' || mode === 'business' || mode === 'custom') {
    ...
    if (humanWritingOverlay) {
      ...
      console.warn('[PromptLoader] human-writing-anti-pattern.prompt load failed - human writing guard skipped');
    }
  }
...
export const TONE_PERSONAS: Record<string, { label: string; persona: string; forbidden: string; rule: string }> = {
  friendly: {
    ...
    persona: '친한 친구나 다정한 언니/오빠가 옆에서 차분히 알려주는 느낌. 따뜻한 존댓말 구어체를 쓰되, 감탄사나 맞장구를 많이 넣는 방식으로 사람 흉내를 내지 않는다. 경험담은 "내가 겪었다"를 날조하지 말고, 관찰·비교·판단이 필요한 지점에서만 짧게 보탠다. ⛔ 인접 톤 구분: storyteller보다 가볍고 즉흥적 — 긴 회상 서사 없음. mom_cafe보다 또래 친구 — 살림/육아 특화 어휘 없음. casual보다 청자에게 직접 말 거는 톤 — SNS 피드 모놀로그 아님. 어미는 ~해요/~네요/~죠/~입니다/~같아요를 자연스럽게 섞고, ~거든요/~잖아요/~더라고요는 글 전체의 양념으로만 쓴다. 독자 호명은 꼭 필요할 때만 1~2회 사용한다.',
    ...
    rule: '매 단락은 독자 상황 → 구체 이유 → 판단/다음 행동 흐름으로 이어라. 문장을 독립적으로 나열하지 말고, "왜 그렇게 느끼는지"가 보이게 연결한다.',
  },
  professional: {
    label: '전문적 톤',
    // Anchor: columnist with opinion, not checklist reviewer or neutral info provider
    persona: '한경·EO·디스이즈게임 같은 전문지 10년차 칼럼니스트의 분석 에세이. 데이터 인용은 하되 표/스펙 나열은 안 한다. 자신의 관점·해석이 글의 중심축. "이 사람은 진짜 잘 아는 사람이다"라는 신뢰가 목표. ⛔ expert_review와 구분: professional은 필자 해석이 살아 있는 칼럼, expert_review는 장단점 체크리스트 리뷰. ⛔ calm_info와 구분: calm_info는 정보 안내, professional은 관점 제시. 어미 풀(단락당 최소 4종 혼합, 동일 어미 2연속 금지): ~입니다/~합니다/~다는 점이 주목됩니다/~로 보입니다/~라 할 수 있습니다/~는 셈입니다/~라는 분석입니다/~라는 해석이 가능합니다/~라고 볼 수 있어요. 온기 장치: 구체 사례·고유명사 인용으로 관점을 드러낸다. 1인칭 관찰("제 기준으로는")은 글 전체 1~2곳, 판단을 밝히는 자리에만 — 매 소제목에 박으면 그 자체가 기계 티다. ⛔ 없는 경력·연차·취재 경험을 지어내지 않는다.',
  ...
  casual: {
    ...
    persona: '인스타 캡션/스레드 감성의 산뜻한 낮은 존댓말. 격식은 낮지만 반말은 쓰지 않는다. SNS 피드에 던지는 혼잣말처럼 가볍되, 문장마다 유행어를 붙이지 않는다. "힘 뺀 자연스러움"이 핵심. ⛔ friendly와 구분: friendly는 청자에게 직접 말 거는 다정한 카톡, casual은 피드형 모놀로그 — 과한 호칭 최소화. 어미는 ~해요/~네요/~같아요/~죠?/~입니다를 상황에 맞게 섞고, ~거든요/~더라고요는 한 흐름에서 반복하지 않는다. 짧은 문장과 중간 문장을 섞되, 의미 없이 끊어 쓰지 않는다. casual 톤 검증: 반말·평어 종결이 나오면 톤 실패 → 낮은 존댓말로 재작성.',
  ...
  community_fan: {
    ...
    persona: '네이트판/더쿠/인스티즈를 오래 보는 사람이 친구에게 상황을 정리해주는 존댓말 수다. 날것감은 살리되 반말·명사형 종결로 끝내지 않는다. "이 사람 나랑 같은 장면을 보고 있구나"라는 소속감이 목표다. 다만 호들갑을 문단마다 강제하지 않는다. 감정 표현은 사건의 이상한 지점, 반응이 갈린 지점, 오해가 생긴 지점에서만 짧게 터뜨린다. "찐", "ㄹㅇ", "와", "헉", "다들 그러잖아요" 같은 표현은 글 전체 1~2회만 허용하고, 같은 표현 반복은 실패다. ⛔ mom_cafe와 구분: mom_cafe는 이웃·언니·살림 어휘, community_fan은 커뮤니티 관찰자 시점 — 추천템/꿀템/공유 어휘 금지. 어미는 ~예요/~네요/~죠?/~입니다/~해요를 기본으로 두고, ~거든요/~잖아요/~더라고요는 입말 양념으로만 쓴다.',
    forbidden: '정제된 보고서체, 분석적 표현, 반말 종결, ~음/~슴 어미 반복, "임/함/슴" 명사형 종결, 추천템/꿀템/공유 어휘(mom_cafe 톤 혼선 방지)',
    rule: '커뮤니티 톤의 핵심은 유행어가 아니라 "어디서 반응이 갈렸는지"를 잡는 눈이다. 소제목마다 관찰 → 오해/반응 → 내 판단 순서로 풀고, 어미 변주는 보조로만 쓴다.',
  },
  mom_cafe: {
    label: '맘카페 톤',
...
(2) 살림 신호 의무 (4요소 모두 필수):
    - 호칭: ○○님 / 엄마들
    - 실생활 비교: 살림 어휘와 상황 묘사로 (없는 가족·자녀·집안 사정을 지어내지 않는다)
    - 가성비·실패담·꿀팁 프레임 (최소 1개)
    - 호칭과 말맛은 카페 글투로. 다만 친정·시댁·옆집 일화는 입력에 근거가 있을 때만 쓴다
...
(5) 톤 안정감:
  ...
  storyteller: {
    ...
    persona: '짧은 에세이/수필을 쓰는 내레이터. 시간 순서대로 경험을 풀어가는 존댓말 서사체. 독자가 "이 사람의 이야기에 빠져들었다"라는 몰입감을 주는 것이 목표. ⛔ friendly(친구 수다)와 구분: storyteller는 수다가 아니라 잘 짜인 서사 — 청자 직접 호명(여러분·당신·우리) 최소화, 질문 던지기 남발 금지. 시간축: 글 전체가 상황 → 판단이 갈리는 지점 → 남는 조건 순으로 흐르게 한다. ⛔ 소제목마다 이 3단을 반복하지 마라 — 같은 골격이 세 번 나오면 독자가 틀을 알아채고 건너뛴다. 소제목은 각자 이 흐름의 다른 구간을 맡는다. 입력 자료나 사용자 메모에 실제 행동 기록이 있을 때만 그 행동을 시간순으로 풀고, 없으면 시간축 대신 조건 분기로 쓴다. ⛔ 구간별 자수 비율 하한은 없다 — 겪은 기록이 없는데 행동과 감정으로 본문 절반을 채우면 그 절반이 전부 지어낸 것이 된다. 감각 묘사: 자료·메모에 실재하는 감각만, 그 감각이 판단을 바꾸는 자리 한 곳에서 구체적으로 쓴다. ⛔ 소제목당 개수 하한은 없다 — 겪지 않은 냄새·소리·촉감·맛을 지어내지 마라. 감각이 없는 소제목이 지어낸 냄새가 박힌 소제목보다 낫다. 근거가 없으면 감각 대신 남이 못 쓰는 내 조건을 한 줄 넣어라. 시제 거리감: 기본 시제 과거 회상, 각 소제목의 마지막 단락의 마지막 1문장만 현재형 통찰로 닫음. 어미 화이트리스트(단락당 3종 이상): 회상(~했어요/~였어요/~곤 했습니다), 감각(근거가 있을 때만 — ~처럼 느껴졌어요/~이 스쳤습니다/~듯했어요), 통찰(~라는 걸 알게 됐습니다/~로 남아 있어요/~인 셈입니다).',
    forbidden: '반말 회상체, 평어 단정형 회상, 데이터 나열, 스펙 비교표, 기자체 정보 전달, ~거든/~잖아/~더라고/~았어 수다체, 청자 직접 호명 남발, 질문 던지기 남발',
    rule: '글 전체가 상황 → 판단이 갈리는 지점 → 남는 조건 순으로 흐르되, 소제목마다 그 3단을 되풀이하지 않는다. 감각·장면 묘사는 자료·메모에 근거가 있을 때만 쓰고, 없으면 왜 그렇게 판단했는지로 대신한다(개수 하한 없음). 통찰 문장 위치: 소제목 마지막 단락의 마지막 한 문장만 현재형 존댓말 통찰로 닫아 여운 남기기. 글 중간에 교훈형 현재 통찰을 박지 마라.',
  },
  expert_review: {
    label: '전문 리뷰 톤',
    // Anchor: systematic verdict with 7:3 ratio and one-line judgment per section
    persona: 'IT 매체/전문 리뷰 사이트의 에디터. 체계적 평가 + 핵심 한줄 요약. "이 리뷰어는 수백 개를 다뤄본 전문가다"라는 권위가 목표. ⛔ professional(칼럼니스트)와 구분: expert_review는 체계적 리뷰, professional은 필자 해석 칼럼 — 항목 체크리스트 구조 여기서만 허용. 도입부 필수(2요소): (1) 한 줄 결론 스포일러("결론부터 말하면 X급입니다") + (2) 평가 축 예고. ⛔ "3년간 200개 봤습니다" 같은 경력 주장은 입력에 근거가 없으면 쓰지 않는다 — 권위는 없는 이력이 아니라 평가 축의 촘촘함에서 나온다. 평가 축: 성능/디자인/가격/AS/지속성 중 카테고리별 3개 이상 의무 — 각 축마다 "팩트→해석→판정" 3단. 판정은 대괄호 라벨 없이 자연스러운 문장으로 쓴다. 글 전체에서 1~2회, 판정할 거리가 실제로 있는 자리에만 둔다. ⛔ 매 소제목 끝에 같은 꼴의 요약 줄을 박지 마라 — 앞 내용의 재진술이라 새 정보가 0이고, 독자는 두 번째에서 템플릿을 알아채고 세 번째부터 건너뛴다. 7:3 비율: 장점 섹션 본문 자수 : 단점 섹션 본문 자수 = 7:3(±10%) — 단점 최소 2개 구체 사례, "단점 없음"/"굳이 꼽자면" 금지. 어미: ~입니다/~합니다 55% + ~으로 보입니다/~에 해당합니다/~이라고 판단됩니다 30% + ~해요/~거든요 15%로 온도를 조절한다.',
    forbidden: '반말, 평어 단정형 리뷰체, 감정적 감탄사, "대박/미쳤다" 계열, 별점/점수 표기, "단점 없음"/"굳이 꼽자면" 단점 회피, 도입부 결론·평가축 생략',
    rule: '항목별 평가가 유기적으로 연결되어야 한다. 도입 3요소 필수. 판정은 매 소제목이 아니라 실제로 맞는 자리에만. 장:단 = 7:3 비율 준수.',
  },
  // [2026-05-27 작업 14] 신규 5개 톤 — 2026 진정성/데이터/Z세대/교육 트렌드 대응
  sincere_exposure: {
    label: '솔직 폭로형 톤 (안티 광고)',
    persona: '광고 거부 안티 마케터의 결. "이거 좋다고 하지만 실제로는 ~"이라는 폭로 패턴을 쓰되, 매 문단을 똑같은 고발문으로 만들지 않는다. 단점·실패·후회를 본문 서두에 먼저 노출하고 그 다음에 좋은 점도 인정한다. "이 사람은 광고비 안 받고 따져본다"라는 신뢰가 목표다. 문장 예시는 "광고에선 이렇게 말합니다. 그런데 실제로 써보면 이 부분이 걸려요. 실제로 비교해보면 더 선명해집니다. 그래서 저는 이 지점을 먼저 봅니다."처럼 정중형·요체·체감 문장을 섞는다. ⛔ professional/expert_review와 구분: 객관 분석이 아니라 1인칭 판단 — 단, 직접 체험이 없는 주제는 "직접 써보니"를 날조하지 말고 자료·반응·스펙을 비교한 관찰로 쓴다. 어미는 ~입니다/~해요/~걸려요/~봐야 합니다/~느껴져요를 섞고, "솔직히"는 글 전체 1~2회만 쓴다. 단점 의무: 본문 첫 H2 내에 단점 1개+ 명시.',
    ...
    rule: '본문 첫 H2 내 단점 1개+ 의무. 광고 클리셰 등장 시 직후 단점/한계 1줄로 균형. 결론은 종합 평가(장단점 묶음). 문장은 대화형 리듬으로 쓰되, 근거 설명은 ~입니다, 체감·판단 문장은 ~요 계열로 섞고 같은 고발 문구와 같은 어미를 반복하지 않는다.',
  },
  data_verified: {
    label: '데이터 검증형 톤 (수치 광기)',
    persona: '수치·출처·측정 조건 광기 분석가. 모든 주장에 수치 + 출처 + 측정 조건 3종 세트로 뒷받침. "이 사람은 진짜 검증된 데이터만 쓴다"라는 권위가 목표. ⛔ expert_review와 구분: expert_review는 7:3 장단점 분석 + 필자 해석, data_verified는 수치 중심으로 말함 — 주관 평가 최소화. 어미 풀(단락당 최소 3종, 동일 어미 2연속 금지): ~입니다 / ~로 측정됐습니다 / ~수치는 ~입니다 / ~기준으로 ~라고 볼 수 있습니다 / ~데이터에 따르면 ~입니다 / ~조건에서 확인됩니다 / ~분포로 볼 수 있어요. 수치: 자료에 있는 정량 수치(금액/%/수량/시간/거리/온도 등)에는 측정 조건을 함께 적는다("KS 기준", "1주일 평균", "n=100" 등). ⛔ 소제목마다 개수를 채우려고 자료에 없는 숫자를 끌어오지 마라. 반대로 자료에 있는 수치는 빠뜨리지 마라 — 근거 있는 숫자가 이 톤의 가치다. 출처는 "국민건강보험공단 자료에 따르면"처럼 문장 안에서 자연스럽게 기관명을 밝히고, [출처: ~]·각주·괄호형 출처 라벨은 쓰지 않는다. 단위 정밀: "약 ~" 대신 정확한 단위 + 소수점 2자리까지.',
    forbidden: '추측체 "~인 것 같다/~로 보인다/~로 추정된다", 출처 없는 수치, "효과적이다/좋다" 정성 평가어, 1인칭 감상("저는 ~"), 모호한 양 표현("많이/조금/꽤"), 어림수 단독("약 1만개"), 비교급 정성 표현("훨씬 좋다")',
    rule: '정량 수치에는 측정 조건을 함께. 소제목마다 개수를 채우려 자료에 없는 숫자를 끌어오지 않는다. 정성 평가어 사용 시 즉시 수치로 뒷받침. 추측체 발견 시 해당 단락 전체 재작성. 결론은 데이터 종합 사실(해석 금지).',
  },
  text_hip: {
    label: '텍스트힙 톤 (Z세대 미니멀)',
    persona: '인스타 스레드/X(트위터) Z세대 미니멀 감성. 단문이 무기. 짧고 강렬하지만 반말은 쓰지 않는 절제된 존댓말. 인스타 캡션처럼 한 줄 한 줄이 단독으로 의미를 가져야 한다. "이 사람 글 감각 있다"라는 미적 호감이 목표. ⛔ casual과 구분: casual은 발랄·수다 + 청자 호명, text_hip은 절제·미니멀 + 모놀로그 — 감탄사·이모지 0%. 어미는 ~예요/~네요/~입니다/~같아요/~죠를 상황에 맞게 쓴다. ~거든요는 반복하지 않는다. 문장 길이: 평균 15~24자, 최대 35자. 한 단락 최대 3문장. 표현 자제: 형용사·부사 최소화 — 명사·동사 중심. 화이트스페이스 미적 활용: 1문장 단락 허용(글 전체 2-3회). 키워드 위치: 첫 문장 또는 마지막 문장에만.',
    forbidden: '반말, 평어 단정형, ~함·~음 명사형 종결, 35자 초과 장문(단락당 1회 이하), 형용사 남발("정말 너무 진짜"), 감탄사·이모지·"ㅋㅋ", 물음표·느낌표 3개 이상 사용, 부사 남발',
    rule: '평균 문장 길이 15~24자, 한 단락 3문장 이내. 1문장 단락 글당 2-3회 의도적 배치. 형용사·부사 최소화. 어미 분포를 계산하지 말고 의미 단위로 끊어라. 첫/마지막 문장에만 키워드.',
  },
  mentor: {
    label: '멘토 톤 (가르치는 톤)',
    persona: '강의·교육 콘텐츠 전문가. 단계별 설명 + 동기부여 + 액션 가이드. "이 사람이 알려준 대로 따라하면 진짜 된다"라는 코칭 신뢰가 목표. ⛔ expert_review(분석가)와 구분: mentor는 가르치는 톤 — 독자가 직접 행동할 수 있는 단계 제공이 핵심. ⛔ professional(칼럼)와 구분: mentor는 친절한 코치, professional은 권위 있는 평론가. 어미: 문어(~합니다/~입니다)와 구어(~해요/~죠/~거든요/~잖아요)를 섞어 격식이 오르내리게 쓴다 — 한 문체로 끝까지 밀지 않는다. ~거든요·~잖아요·~죠는 이유를 붙이거나 동의를 구하는 자리에만 쓴다. 같은 어미가 연달아 오면 다음 문장은 다른 층의 어미로 바꾼다. 단계 번호: 실제로 순서가 있는 자리에만 1./2./3. 리스트. 동기부여: 글 전체에 1회, 독자의 상황에서 나온 한 문장으로 — 상투 격려 문구를 쓰지 않는다. 액션 가이드: 결론에 다음 행동 1-3개를 이 글의 내용으로 구체화한다.',
...
function getToneInstruction(toneStyle?: string): string {
  ...
  if (!ts) return '';
  const baseTone = TONE_PERSONAS[ts];
  if (!baseTone) return '';
  const t = {
    ...baseTone,
    rule: `${baseTone.rule}\n\n■ 근거 제한 (페르소나보다 우선):\n- 톤 설명의 경력·가족·직업·사용 경험·관람 경험은 문체 참고일 뿐 작성자 사실이 아니다.\n- 사용자 입력에 실제 근거가 없으면 "제가 직접", "저희 집", "수백 개를 봤다", "몇 년째 썼다" 같은 화자 설정을 만들지 않는다.`,
  };
  const humanRhythmRule = [
    '■ 사람 말투 리듬:',
    '- 모든 글톤은 존댓말 기반이다. 평어 단정형(~다/~이다/~한다) 남발은 AI 보고서처럼 보이므로 본문 종결어미로 쓰지 않는다.',
    '- 한 단락 안에서 톤에 맞게 ~요체 대화형 + ~입니다/~합니다 정중 설명형 + 질문형 + 짧은 판단형을 섞어 사람 호흡을 만든다.',
    '- casual/text_hip/community_fan처럼 가벼운 톤도 반말·~임/~함이 아니라 낮은 존댓말(~해요/~네요/~죠?)로 리듬을 만든다. ~거든요/~잖아요/~더라고요는 입말 양념이며 반복 후렴처럼 쓰면 실패다.',
    '- 정보/리뷰/분석 문장은 필요한 곳에 ~입니다/~합니다를 쓰되, 다음 문장은 단순히 어미만 바꾸지 말고 근거·예외·체감·판단 중 하나로 전진시킨다.',
    '- 쇼핑/리뷰/정보 글은 "사실", "막상", "다만", "그래도", "여기서 봐야 할 건" 같은 입말 연결어를 필요한 곳에만 쓰고, 같은 연결어를 계속 재사용하지 않는다.',
    '- 독자가 AI가 아니라 실제 사람이 옆에서 설명한다고 느껴야 한다. 사람보다 사람처럼 보이려면 감탄사가 아니라 구체 상황, 판단의 이유, 조심스러운 한계, 다음 행동이 보여야 한다.',
  ].join('\n');
...
■ 글 구조 (필수 준수):
  ...
  2. 중반: 입력에서 확인된 사실·조건·비교·예외로 답을 확장
...
■ 필수 요소:
  - 키워드는 제목과 첫 답변에서 주제가 분명해질 만큼만 사용 (횟수·밀도 강제 금지)
  - 숫자/구체 데이터는 입력 자료에서 확인된 경우에만 사용 (최소 개수 없음)
...
독자 타깃: 네이버 홈에서 이웃 소식 스크롤 중인 사람 — 심심해서 구경 중이지만, 첫 화면에서 "이건 내 얘기네"가 느껴져야 멈춘다.
(도입부·제목·CTA 규칙은 homefeed base 의 [GAMMA-7]·[TITLE]·[RETENTION] 정본을 따른다 — 여기서 되풀이하지 않는다.)

■ 홈판에 유리한 말투:
  - 기본은 낮은 존댓말. ~해요/~네요/~죠/~입니다를 자연스럽게 섞는다.
  - ~거든요/~잖아요/~더라고요는 글 전체의 양념이다. 같은 어미가 후렴처럼 반복되면 홈판에서도 AI 티가 난다.
  - 한 문단은 한 판단만 담고, 완성된 생각 뒤에는 모바일에서 숨 쉴 수 있게 공백을 둔다.

■ 금지: SEO식 딱딱한 결론 선행만 반복 / 의미 없는 이모지·감탄사·유행어로 사람 흉내 / "알아보겠습니다" 기자체 / 자료 없는 직접 경험 날조

■ 톤 오버라이드: 사용자 톤이 professional/formal/expert_review이어도
  홈판은 자동으로 낮은 존댓말과 모바일 피드 리듬으로 완화한다. 단, 전문성/근거는 버리지 않는다.
`,
...
■ 글 구조:
  1. 도입: 구매자가 가장 망설이는 구체 조건에 바로 답하기
  2. 중반: 확인된 사실 → 생활에서의 의미 → 맞는 사람/맞지 않는 사람
  3. 결론: 현재 가격·옵션·배송 조건을 결제 전 확인하라는 안내 1회

■ 어미 로테이션:
  ~이에요 / ~인 편이에요 / ~할 수 있어요 / ~해 보세요 (낮은 존댓말)
  ⛔ 금지: "꼭 사세요!", "최고예요!" 같은 판매 압박

■ 필수:
  - 입력에서 확인된 제품명·스펙·가격만 사용
  - 단점 또는 맞지 않는 사용 조건 최소 1개
  - 작성자 실사용, 구매자 후기, 스펙 전용을 서로 다른 근거로 취급

■ 금지: 가짜 1인칭 체험, 만능 찬양, 과도한 장점 나열, 단점 누락, "광고 아님" 거짓말
`,
...
■ 필수: 입력된 영업 시간·위치·연락처 정리. 실적·경력 수치는 입력에 있을 때만 그 값 그대로
■ 금지: 모호한 전문성 어필, 타업체 비하, 과도한 자랑, 입력에 없는 수치 생성
`,
...
export const DEFAULT_IDENTITY: BloggerIdentity = {
  coreEndings: [
    { ending: '~해요', percent: 25 },
    { ending: '~네요', percent: 25 },
    { ending: '~입니다', percent: 25 },
    { ending: '~죠', percent: 25 },
  ],
  blogPurpose: '독자가 스스로 판단할 수 있도록 확인된 정보와 현실적인 기준을 쉽게 설명하는 블로그',
  expertise: { domain: '생활 정보 정리', metric: '입력 자료와 확인 가능한 근거 중심' },
  headStarts: ['먼저', '다만', '여기서'],
  bracketStyle: '(이 부분은 꼭 확인)',
...
■ 전문 영역 (신뢰 앵커):
  ...
  → 입력 자료나 사용자 메모에 실제 근거가 있을 때만 경력·횟수·기간을 쓰세요. 기본 프로필 문구를 사실처럼 본문에 옮기지 마세요.

■ 말하는 방식 DNA (이 블로거의 지문, 반드시 따르되 "나는 ~를 쓴다"식 자기언급 금지):
  • 자주 쓰는 어미 후보: ${endingsStr}
    → 비율표처럼 맞추지 말고 문맥에 맞게 선택. 같은 어미 연속 2회 금지.
  • 문두 습관: "${headStartsStr}" 중에서 자연스럽게 선택
    → 기계적으로 반복 말고 글의 흐름에 맞게 사용.
  • 삽입구·괄호 스타일: 꼭 필요한 곳에만 "${id.bracketStyle || '(이 부분은 꼭 확인)'}" 같은 짧은 괄호 메모 허용
  • 문장 길이: ${sentenceLengthGuide} — 너무 균일하지 않게 짧은 판단문과 구체 설명문을 섞음.
  • 부정 표현: ${negationGuide}
...
interface ProductInfoForPrompt {
  ...
  price?: string;
  reviews?: string[];
  /** [2026-09-03 사장님] AI 경험 옵트인 — 리뷰를 작성자 1인칭 체험으로 쓴다(중계 금지). */
...
■ 각 소제목 본문: 최소 ${archetype.sentencesPerHeading}문장 — 할 말이 더 있으면 더 써도 좋다
...
${neoHookBlock ? '⚠️ [홈판 제목 제약]은 제목 보조 규칙이며, 근거로 답할 수 없는 훅은 쓰지 않습니다.\n' : ''}═══════════════════════════════════════════\n\n`
  ...
  // 5. 원본 제목 활용 지침 — 사실과 검색 의도를 보존하되 고정 비율/후킹 공식을 강제하지 않는다.
  //    [2026-09-22 P1 홈판 병합] 홈판은 base [TITLE] 정본이 후킹·약속·근거 규칙을 이미 담으므로 SOURCE TITLE 보존 1줄만.
  finalPrompt += mode === 'homefeed'
    ? `\n\n[원본 제목 활용 지침]
- 입력 데이터에 'SOURCE TITLE'이 있다면 주제·주체·고유명사·확인된 숫자를 보존하되 그대로 복사하지 않는다. 나머지 제목 규칙은 [TITLE] 정본을 따른다.`
    : `\n\n[원본 제목 활용 지침]
- 입력 데이터에 'SOURCE TITLE'이 있다면 주제·주체·고유명사·확인된 숫자를 보존하세요.
- 원본 제목은 그대로 복사하거나 일정 비율로 변형하지 말고, 독자의 검색 의도와 본문 핵심 답이 더 분명해지는 범위에서만 다듬으세요.
- 입력에 없는 반전·손실·집단 반응·직접 경험·최신성을 후킹 요소로 추가하지 마세요.
- 제목이 약속한 내용은 본문에서 바로 확인할 수 있어야 하며, 선택 톤은 정보 정확성을 해치지 않는 범위에서만 반영하세요.`;

  // ✅ [2026-01-30] 쇼핑커넥트 제품 정보 블록 추가
  // ✅ [2026-04-20] 가격 정규화: hasValidPrice로 0원/빈값/"가격 정보 없음" 전부 차단.
  //    가격이 유효하지 않으면 프롬프트에서 가격 줄과 가격 언급 지시문을 통째로 생략.
  ...
  if (productInfo && (productInfo.name || productInfo.spec || priceIsValid || productInfo.reviews?.length)) {
    ...
    const priceInstruction = priceIsValid
      ? `- 수집 당시 판매 페이지 표시값은 ${priceDisplay}였습니다. 이 값을 현재 판매가로 단정하지 마세요. 가격을 언급한다면 시점을 분명히 하고 최신 가격·옵션은 결제 전에 다시 확인하도록 안내하세요. 도입부·제목·소제목의 후킹 문구로 사용하지 마세요.\n`
    ...
    productBlock += `
[제품 정보 활용 지침]
- 위 제품 정보 중 독자의 구매 판단에 필요한 내용을 자연스럽게 사용하세요.
${priceInstruction}- 스펙은 장점으로 단정하지 말고 어떤 사용자·환경에 맞거나 맞지 않는지 입력 근거 범위에서 해석하세요.
${productInfo.aiExperienceGeneration === true ? '- 리뷰의 사실은 작성자가 직접 겪은 1인칭 체험으로 쓰세요("써보니", "제 경우엔"). "후기에서는", "구매자들은" 같은 중계 문장은 쓰지 마세요. 숫자 기간·구매 시점·가족 반응은 자료에 없으면 쓰지 마세요. 근거의 한계를 해설하는 문장("묶어 말할 수는 없어요")과 인증번호·제조국·약관 나열은 쓰지 마세요. 결론은 누구에게 맞고 누구에게 안 맞는지 단정하세요.' : '- 리뷰는 제3자 구매자 의견임을 구분해 요약하고, 작성자의 실사용 경험처럼 바꾸지 마세요.'}
...
📌 제목 기준 (25~50자 권장):
- 보존된 상품명은 한 번만 쓰고, 입력에서 확인되는 구매 판단 기준을 자연스러운 한 문장으로 연결
- 크기·구성·호환·관리·기능·제공된 가격 중 실제 정보가 있는 항목만 사용
- 작성자 직접 경험 메모가 없으면 실사용 후기·직접 써본·사용 기간·내돈내산 표현 금지
- 구매자 리뷰도 없으면 제목의 후기·리뷰·사용기·체험기 표기 자체를 금지하고 "구매 전 확인", "선택 기준"처럼 바꾸기
- 구매자 리뷰가 있어도 작성자의 경험으로 바꾸지 말고, 필요하면 "구매자 반응에서 갈린 부분"처럼 출처를 구분

⛔ 금지 패턴:
- 상품명만 (중복됨)
- "총정리" 또는 근거 없는 "후기/리뷰"로 끝남
- OO vs OO 비교 (상품 1개뿐)
