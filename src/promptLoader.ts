...
export const TONE_PERSONAS: Record<string, { label: string; persona: string; forbidden: string; rule: string }> = {
  ...
  community_fan: {
    label: '찐팬/커뮤니티 톤',
    // ✅ [v2.10.226] 사용자 보고: "GPT로 찐팬모드 하니까 음슴체만 쓴다"
    //   원인: 프롬프트가 "~임/~함/~거든요"만 명시 → GPT가 ~음/~슴 어미로 over-fit.
    //   수정: 어미 풀(pool)을 8개로 늘리고 "동일 어미 2문장 이상 연속 금지" 명시.
    persona: '네이트판/더쿠/인스티즈 찐팬이 실시간으로 호들갑 떠는 수다. 날것 그 자체. "이 사람 나랑 완전 같은 편이다"라는 소속감이 목표. 어미 풀: ~임/~함/~음/~거든요/~잖아요/~더라고요/~네요/~인 듯/~한 것 같아요 — 이 9개를 골고루 섞어 써라. 동일 어미 2문장 이상 연속 사용 금지(특히 ~음/~슴 단조 반복은 피로 유발). 한 단락 안에서 최소 3가지 어미를 섞을 것.',
    forbidden: '~습니다/~합니다 등 격식체, 정제된 문장 구조, 분석적 표현, ~음/~슴 어미만 단조 반복(2문장 연속 시 즉시 다른 어미로 교체), "임/함/슴" 모음 어미만 줄줄이 늘어놓기',
