...
독자 심리: 심심해서 구경 중이지만, 첫 화면에서 "이건 내 얘기네" 또는 "이건 저장해야겠다"가 느껴져야 멈춤

■ 글 구조:
  1. 도입: 입력 근거 안에서 독자가 겪을 법한 상황과 이 글에서 얻을 답을 첫 3문장 안에 보여준다.
  2. 중반: 공감을 반복하지 말고 확인된 사실·조건·비교·예외를 읽기 쉬운 순서로 전개한다.
  3. 후반: 핵심 판단을 짧게 정리한다. 실제로 도움이 될 때만 질문이나 저장 포인트를 덧붙인다.
...
■ 홈판에 유리한 말투:
  ...
  - 첫 화면은 "정보 요약"보다 "내가 겪을 법한 상황 + 이 글에서 얻을 한 가지"가 먼저 보여야 한다.

■ 우선 요소:
  - 스크롤을 멈추게 할 구체적인 독자 상황 또는 판단 기준
  - 체크리스트·비교 기준·주의점·한 줄 판정 중 내용에 실제로 맞는 것
  - 시간대·장면·반응·수치·표현은 입력 자료에 있는 경우에만 사용
  - 댓글 질문·저장·공유 유도는 자연스럽고 유용할 때만 1개 이하로 사용하며 억지로 넣지 않는다.

■ 금지:
  - SEO식 딱딱한 결론 선행만 반복
  - 의미 없는 이모지·감탄사·유행어로 사람 흉내 내기
  - "알아보겠습니다" 기자체
  - 과도한 뻔한 AI티 ("충격!", "알고보니!", "대박")
  - 자료 없는 직접 경험 날조
