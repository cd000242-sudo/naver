...
- review_synthesis에서는 상품명과 누구나 아는 기능 설명보다 리뷰에 나타난 설치 난점, 반복 불편, 소음·관리·공간 변수, 사용 뒤 해결된 문제를 우선한다.
- 전환 흐름은 독자의 구체 상황 → 후기에서 확인된 사용 결과 → 그래서 어떤 사람에게 왜 맞는지 순서다. 작성자의 직접 경험으로 바꾸지 않는다.
- 후기의 품질·범위·개수를 평가하는 문장을 본문에 쓰지 않는다. 근거 없는 항목은 언급 없이 생략한다.
- 반론은 source_data_json에서 구체적으로 확인된 항목만 다루고, 근거 없는 유형은 언급조차 하지 않는다. 고정된 가격·설치·관리 체크리스트를 만들지 않는다.
- 제품을 사지 않았을 때 남는 시간·공간·반복 노동·비용·스트레스는 근거가 있을 때만 설명하고 공포나 불편을 과장하지 않는다.
- 각 리뷰 근거는 구체 상황 → 실제 사용 결과·해결·적응 또는 남은 한계 → 구매 전에 중요한 이유 → 맞는 사람/맞지 않는 사람으로 연결한다. 2건 이상이 뒷받침할 때만 반복 의견이라고 말한다.
- 가장 잘 맞는 사람과 맞지 않는 사람을 각각 구체적으로 좁힌다. 결론에는 CTA 직전 전환 문장만 한 번 두고, 실제 CTA와 URL은 앱이 보존한 원본 제휴 링크로 삽입하므로 모델이 URL을 출력·생성·교체·변경하지 않는다.
