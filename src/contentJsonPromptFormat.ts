...
/*
 * [2026-09-01] evidence 필드를 신설한다.
 *
 * 사장님 요구: "십중팔구는 완벽히 나와야 돼." 그리고 조건이 붙었다 —
 * "검사를 해서 다시쓰기를 하면 의미가 없어. 비용만 들고 차라리 LLM을 쓰고 말지."
 * 맞다. 생성 후 재작성은 비용을 두 배 내고도 raw LLM 보다 나을 이유가 없다.
 * 첫 호출에서 지키게 해야 한다.
 *
 * 이 저장소에서 첫 호출을 바꾸는 레버는 스키마 필드 하나뿐이다. 오늘 하루만
 * 산문 지시가 무시된 것을 다섯 번 확인했고(소제목 문형 · 요약표 규율 · 매 섹션 템플릿 ·
 * 연도 표기 · 자료 라벨), 스키마 필드로 올린 것은 지켜졌다(해시태그 개수 · 제목 길이 ·
 * 요약표 존재 · clickReason).
 *
 * 모델이 수치를 쓰려면 그 수치가 적힌 자료 문장을 함께 뱉어야 한다면,
 * 뱉을 수 없는 수치는 애초에 쓰지 않는다. 추가 호출이 없으므로 비용은 0이다.
 *
 * 실측: 클로드코드로 뽑은 두 편에서 자료에 없는 수치가 각각 6건 · 5건 나왔다
 * (2번 · 3번 · 2020년 · 30분 · 22일). 에이전트 모드는 검색이 막혀 자기 지식으로 쓴다.
 */
function buildHeadingsExample(): string {
  return `"headings": [
    {"title": "소제목 1", "content": "본문 내용...", "summary": "요약", "keywords": ["키워드"], "imagePrompt": "이미지 프롬프트", "evidence": ["이 섹션에서 쓴 수치·날짜·기관명이 적힌 자료 문장을 그대로 옮겨 적는다"]},
    {"title": "소제목 2", "content": "본문 내용...", "summary": "요약", "keywords": ["키워드"], "imagePrompt": "이미지 프롬프트", "evidence": []},
    {"title": "소제목 3", "content": "본문 내용...", "summary": "요약", "keywords": ["키워드"], "imagePrompt": "이미지 프롬프트", "evidence": []}
  ]`;
}
...
- [훅은 말이 되어야 한다] 제목만 읽고 0.5초 안에 무슨 이야기인지·왜 눌러야 하는지 파악되지
  않으면 그 후보는 버린다. 억지 어미, 의미 불명 생략, 문법이 깨진 훅도 후보에서 버린다.
...
- [훅은 말이 되어야 한다] 제목만 읽고 0.5초 안에 어떤 질문의 답인지 파악되지 않으면
  그 후보는 버린다. 억지 어미, 의미 불명 생략, 문법이 깨진 훅도 후보에서 버린다.
...
- conclusion: finalVerdict 의 구매 판단으로 돌아와 맞는 사람·안 맞는 사람을 매듭짓고, 구매 압박 없이 현재 가격·옵션·배송 조건 확인을 1회만 안내한다.
- 광고대행사 문구, AI 보고체, 사람 흉내용 감탄사 반복을 금지한다.
`;
  }

  return isHomefeed ? `
⚠️ [홈판 모드 필수 구조 규칙 — JSON 필드 계약] (도입부·소제목·CTA 규칙은 [GAMMA-7]·[HEADINGS: 홈판]·[RETENTION] 정본, 여기는 필드 배치만)
- introduction: 2~4개의 짧은 문장 — [GAMMA-7] 정본대로 답부터 준다.
...
- conclusion: 도입이 던진 질문을 finalVerdict 의 판단으로 매듭짓고, 다음 확인 행동 또는 여운을 1~3문장으로 마무리. 본문을 요약해 되풀이하지 않는다. 댓글·저장·공유를 동시에 요구하지 않는다.
...
- [강제] "한 줄 판정", "한 줄 결론", "최종 판정", "[한 줄 판정: ...]" 같은 라벨 출력 금지. 핵심 문장은 라벨 없이 자연 문장으로 작성
- 결론: finalVerdict 의 판단을 필자의 말로 다시 짚고 + 다음에 확인할 행동 1개. 본문 요약 나열 금지, 선정/수익/AI 브리핑 인용 보장 표현 금지
` : (mode === 'business' || mode === 'custom') ? `
⚠️ [공통 구조 규칙]
- 1번 소제목은 메인 주제(주어)로 시작하고, 첫 1~2문장에서 바로 핵심에 답한다.
- 제목은 자극어 대신 정보 가치와 판단 기준을 약속하고, 그 약속에는 도입부가 직접 답한다.
- conclusion: finalVerdict 의 판단으로 돌아와 매듭짓는다. 본문을 요약해 되풀이하지 않는다.
` : `
⚠️⚠️⚠️ [SEO 모드 필수 규칙] ⚠️⚠️⚠️
- 소제목은 주어가 생략된 채 조사(~의, ~에 대한)로 시작하지 않는다 (예: "아이폰16 디자인" - O / "의 디자인" - X). 첫 소제목을 메인 키워드로 억지로 시작할 필요는 없다 — 검색 질문과 흐름이 우선(base 소제목 원칙).
- conclusion: finalVerdict 의 판단으로 돌아와 매듭짓는다 — 조건이 갈렸으면 어느 조건에서 어떻게 되는지 다시 짚는다. 본문 요약 되풀이·"결론적으로/정리하면" 금지.
...
export function buildContentJsonOutputFormat(options: ContentJsonOutputFormatOptions): string {
  ...
  /*
   * [2026-09-02 실측] 본문 프롬프트에 오늘 날짜가 한 번도 들어가지 않았다.
   *
   * 제목 프롬프트에는 있다(contentGenerator:902 "지금은 N년입니다" + 날짜 규율 2줄).
   * 그래서 제목은 멀쩡했고 본문만 무너졌다 — 사장님 침구 글이 지난해 보도자료의
   * "오는 17일부터" 를 그대로 옮겨, 1년 전에 끝난 행사를 지금 열리는 것처럼 안내했다.
   * 같은 글이 오지 않은 날("9월 26일~10월 9일 매출 25% 증가")을 과거형으로 단정했다.
   *
   * DateClaim 감지기는 그 문장을 정확히 잡았지만 경고만 남고 글은 그대로 나갔다.
   * 감지 후 재작성은 비용만 늘 뿐이다 — 첫 호출에서 알려주는 것이 유일한 해법이다.
   */
  const now = new Date();
...
{${buildPreWritingAnalysisSchema(mode, usesIssueStorySkeleton, todayLabel)}
  ...
  "summaryTable": [
    ...
    {"label": "또 다른 축", "value": "확인된 값", "condition": ""}
  ],`}
  "introduction": "${isHomefeed ? '도입부 (2~4개의 짧은 문장, 상황 + 읽을 이유)' : isMate ? '도입부 (첫 300자 안에 직접 답변)' : '도입부'}",
  "conclusion": "${isHomefeed ? '마무리 (finalVerdict 의 판단으로 돌아와 매듭 + 다음 확인 행동 또는 여운, 1~3문장)' : isMate ? '마무리 (finalVerdict 의 판단을 필자의 말로 다시 짚고 + 다음 행동 1개)' : '마무리 (finalVerdict 의 판단으로 돌아와 매듭)'}",
  "hashtags": ["해시태그1", "해시태그2", "해시태그3"],
  "category": "카테고리"
}

${buildPreWritingAnalysisDirective(mode, usesIssueStorySkeleton)}📌 [근거 인용 — 모든 모드 공통 · 가장 중요]
headings[].evidence 는 그 섹션에서 쓴 수치 · 날짜 · 기관명 · 가격 · 비율이
적힌 자료 문장을 **그대로 옮겨 적는 자리**다. 요약하지 말고 원문 그대로.
⛔ 자료에서 그 문장을 찾을 수 없으면 그 수치를 본문에 쓰지 마라. evidence 를 지어내지도 마라.
   반대로 자료에 있는 숫자·날짜·기관명은 빠뜨리지 않고 쓴다 — 근거 있는 구체적 숫자가 이 글의 가치다. 금지는 '근거 없는 숫자' 하나뿐이다.
✅ 수치를 하나도 안 쓴 섹션은 evidence 를 빈 배열 [] 로 둔다. 그것이 정상이다.
✅ 자료에 없지만 상식으로 아는 것은 수치 없이 쓴다("여러 번", "며칠" 처럼).

📌 [소제목 스타일 — 모든 모드 공통] headings[].title은 30자 이내.
   소제목 전체를 같은 종결형으로 통일하지 않는다 — 명사구·질문형·짧은 서술형을 섞는다.
   ⛔ 전부 "…이유 / …지점 / …조건 / …기준" 처럼 같은 꼴로 끝내면 기계가 쓴 목차로 읽힌다.
...
⚠️ [홈판 모드 제목 필수 조건]
1. 제목 규칙의 정본은 system 의 [TITLE](HOMEFEED_TITLE_RULES) 하나다 — 길이(33~42, 장치 3개 이상이면 초과 허용)·장치·금지어·근거 조건은 그곳만 따른다.
2. 제목과 본문 주제가 1:1로 일치해야 하며, 주제 이탈 금지. 본문에 없는 주제를 제목에 걸면 홈피드 노출 자체가 막힌다.
3. 제목이 만든 궁금증은 도입부가 즉시 갚는다 — 궁금증만 만들고 본문이 답하지 않으면 낚시다.` : ''}
...
8. 오늘은 ${todayLabel}이다. 이 날짜가 자료의 시점을 판정하는 유일한 기준이다.
   · 자료에 적힌 날짜·연도·기간은 그대로 옮긴다. 올해로 바꾸지 않는다.
     자료가 지난해 것이면 지난해 일로 쓴다("2025년 10월에 열린 행사입니다").
   · **"오는 17일", "다음 달", "이번 주" 처럼 자료가 쓴 상대 표현을 그대로 옮기지 않는다.**
     자료의 절대 날짜로 바꾸고, 그 날짜가 오늘보다 이전이면 과거형으로 쓴다.
     지난해 행사를 "오는 17일부터 열립니다" 로 옮기면 독자가 없는 행사를 찾아간다.
   · 오늘 이후의 날짜를 이미 일어난 일처럼 쓰지 않는다.
     아직 오지 않은 기간의 실적·결과·반응을 과거형으로 단정하면 그 자체가 거짓이다.
