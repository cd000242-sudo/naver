...
{${buildPreWritingAnalysisSchema(mode, usesIssueStorySkeleton)}
  ...
  // ⛔ 표는 본문보다 강하게 말하지 않는다. 본문에 조건이 붙은 값은 표에도 조건을 적는다.
  //    (X) "공간의 30%가 늘어난 사례 있음"  ← 본문에서는 "과대포장을 덜어냈을 때의 체감"이라고 유보했다
  //    (O) "과대포장을 덜어낸 경우 30%"     ← 표가 먼저 읽히므로, 조건이 빠지면 독자는 단정으로 받는다
  // ⛔ 글 주제와 직접 상관없는 축은 넣지 않는다. 자료에 있다고 다 넣는 것이 아니다.
  //    (X) 정리·성에 제거 글에 "보관 용량 586L", "에너지 효율 1등급" ← 제품 구매 정보다
  // ⛔ 독자가 자기 상황에 대볼 수 없는 값은 넣지 않는다.
  //    (X) "586L의 4도어 사례 참고 가능" ← 정보도 지시도 아니다. 읽고 할 일이 없다.`}
