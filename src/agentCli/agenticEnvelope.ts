...
const MODE_FOCUS: Record<string, string> = {
  homefeed: `이 글의 목적은 네이버 "홈피드 노출"입니다. 자기비평에서 다음을 특히 끌어올리세요.
   - 첫 화면에서 독자가 겪을 법한 상황과 이 글에서 얻을 한 가지가 바로 보이는가
   - "상황 + 판단"이 입력 근거에 맞는가 — 사용자 경험 메모가 없으면 작성자 체험을 만들지 않았는가
   - 봇 같은 상투어·AI 티가 0에 가까운가 (사람이 직접 쓴 글로 읽혀야 함)
   - 감정어보다 구체 맥락·판단 이유·한계가 살아 있는가
   - 짧은 단락과 변별된 소제목을 지키되 댓글·저장·공유를 기계적으로 모두 요구하지 않았는가
   - [소제목] 검색 목차가 아니라 흐름 표지인가 — 소제목만 위에서 아래로 읽었을 때
     상황 → 갈림길 → 판단으로 이어지는가. 항목 나열로 읽히면 피드에서 스크롤이 지나간다
   - [소제목] 검색어를 앞에 접두로 박은 소제목이 0건인가 ("○○ 조건", "○○ 방법 정리" 형태)
   - [소제목] 만능 라벨("달라진 ○○", "꼭 챙겨야 할 ○○", "총정리", "마무리")이 0건인가`,
  seo: `이 글의 목적은 네이버 "검색 노출(SEO)"입니다. 자기비평에서 다음을 특히 끌어올리세요.
   - 제목과 첫 화면에서 검색 질문의 답과 적용 조건이 분명한가
   - 키워드 횟수·앞배치보다 검색 의도를 충족하는 정보와 소제목 구조를 우선했는가
   - 입력에 있는 근거·조건·예외를 정확히 쓰고, 없는 실경험·수치를 만들지 않았는가
   - 팩트가 정확하고 추정·환각이 없는가
   - [소제목] 각 소제목이 서로 다른 질의 축(정의·조건·절차·비용·비교·예외·확인처)을 하나씩 맡는가 —
     같은 답을 각도만 바꿔 반복한 소제목이 없는가
   - [소제목] 소제목만 위에서 아래로 읽었을 때 그 검색어의 목차가 되는가
   - [소제목] 답을 본문으로 미루는 소제목(정체 숨김·말줄임표·"그 이유는?")이 0건인가 —
     홈판에서는 유효해도 검색 유입에서는 이탈 신호다`,
  affiliate: `이 글의 목적은 "구매 전환"입니다. 자기비평에서 다음을 특히 끌어올리세요.
   - 제품의 가치·차별점이 독자 입장에서 와닿게 설명됐는가
   - 작성자 경험·구매자 리뷰·스펙을 구분해 신뢰를 주는가 (과장·허위 없이)
...
3. 자기비평: 초안을 [작업 명세]의 평가 기준으로 스스로 채점한다. 특히 다음을 정량으로 점검한다.
   ...
   - [제목] 입력 키워드를 그대로 제목에 옮겨놓지 않았는가 — 키워드 복사는 제목을 안 쓴 것이다
   - [제목] 읽는 사람의 상황·조건·시점·갈림길 중 최소 하나가 제목에 들어갔는가
   - [제목] "○○ 총정리 / ○○ 방법 / ○○ 가이드 / ○○ 확인 / ○○ 정리 / ○○ 알아보기"처럼
     주제어 + 템플릿 종결어로 끝내지 않았는가 — 그 자리는 AI 요약이 이미 답했다
   - [제목] titleCandidates 3개가 서로 다른 각도인가 — 같은 틀에 단어만 바꾼 변형은 1개로 친다
   - [본문] 검색하면 누구나 아는 내용을 나열만 하지 않았는가 — 조건이 갈리는 지점, 예외,
     판단 근거처럼 "읽어야 할 이유"가 각 소제목에 하나씩 있는가
