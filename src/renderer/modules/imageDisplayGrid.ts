...
async function useSelectedImageFromPreview(image: any, index: number): Promise<void> {
  try {
    ...
    if (headings.length === 0) {
      // ✅ [v2.8.8] 소제목 없을 때 더 명확한 안내 — 사용자 보고: "소제목이 선택되지 않았다고 뜸"
      alert(
        '⚠️ 소제목이 없습니다.\n\n' +
        '이 기능을 사용하려면 다음 순서로 진행해주세요:\n' +
        '1️⃣ 통합 발행 탭에서 글을 먼저 생성 (URL/키워드 입력 → 글생성)\n' +
        '2️⃣ 글 생성 후 자동으로 소제목 분석이 실행됩니다\n' +
        '3️⃣ 그 다음에 이 폴더 이미지를 소제목에 배치할 수 있습니다\n\n' +
        '※ 글 없이 이미지만 미리 모으고 싶다면 "AI 자동 수집 및 저장하기" 버튼을 사용하세요.'
      );
      return;
    }
