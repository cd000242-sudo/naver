...
/**
 * 새 글 생성을 시작할 때 반자동편집 필드를 비운다.
 *
 * [2026-09-17] 미리보기에는 새 글이 뜨는데 반자동편집 칸에는 이전 글이 그대로
 * 남아 있었다. 생성 시작 블록이 이미지·postId·전역 상태는 비우면서 화면 입력칸만
 * 빠뜨렸고, fillSemiAutoFields 는 생성이 끝나야 값을 덮는다. 그래서 생성 중이거나
 * 실패하면 이전 글이 계속 보이고, 그걸 새 글로 알고 발행할 위험이 있었다.
 * (풀오토 경로에는 같은 초기화가 이미 들어 있다 — fullAutoFlow 의 prevTitleInput)
 */
export function clearSemiAutoFieldsBeforeGeneration(): void {
...
export async function generateContentFromUrl(
  ...
  clearSemiAutoFieldsBeforeGeneration(); // 화면 입력칸도 함께 비운다 (이전 글 잔존 방지)
...
export function fillSemiAutoFields(
  ...
  // [2026-09-17] 미리보기는 차는데 반자동편집 필드만 비는 사고.
  //   updateUnifiedPreview 바로 다음이 이 함수인데, 첫 줄의 JSON.stringify 가
  //   순환 참조를 만나면 TypeError 를 던지고 필드 채움이 통째로 날아간다.
  //   (structuredContent 는 이미지·소제목·표가 붙으며 커지는 객체다)
  //   로그 100자 찍자고 전체 직렬화할 이유가 없으므로 필요한 필드만 찍는다.
  try {
    console.log('[fillSemiAutoFields] Called with:', {
      title: String(structuredContent?.selectedTitle || structuredContent?.title || '').slice(0, 60),
      bodyChars: String(structuredContent?.bodyPlain || structuredContent?.content || '').length,
      headings: Array.isArray(structuredContent?.headings) ? structuredContent.headings.length : 0,
    });
  } catch {
    /* 로그는 실패해도 본 작업을 막지 않는다 */
  }
...
[0단계 — 제목 쓰기 전에 반드시 추론]
① 원본 글에서 독자가 실제로 얻는 답이 무엇인지, 가장 의외이거나 구체적인 지점이
   어디인지 찾는다 (원본에 있는 것만 — 날조 금지).
② 그로부터 clickReason 을 정한다: "지나가던 사람이 이 글을 클릭할 단 하나의 이유".
③ 모든 제목은 그 clickReason 에서 출발한다. 원본 제목을 복사하거나 단어만
   재배열한 제목 = 0점.

[제목 규칙]
- 핵심키워드를 자연스러운 위치에 1회 포함 + clickReason 이 읽히는 구체 상황·판단 기준.
- 요약 명사 종결 금지: "~총정리/~완벽 가이드/~정리/~모음/~현황"으로 끝나면
  보도자료 문형이라 0점 — 궁금증이 남는 서술이나 의문으로 끝낸다.
- 클릭베이트 금지: "충격/소름/경악/난리/울컥" 같은 근거 없는 감정어 0점.
  자극은 원본 속 실제 사실의 의외성에서만 끌어온다.
- [자가검증] 제목을 짓고 "이 제목을 본 사람이 왜 클릭하나?"를 한 문장으로 써 본다.
  자연스럽게 안 써지면 버리고 다시. 제목만 읽고 0.5초 안에 무슨 이야기인지
  파악되지 않으면 0점.
...
□ AI 탐지 금지 표현 0개
□ 사용자 선택 글톤에 맞는 어미 통일
□ 제목이 clickReason 에서 출발했고, 원본 제목 복사·재배열이 아님
□ 제목에 총정리/완벽 가이드/충격/소름류 상투 트리거 없음
□ 도입부 3줄 후킹 패턴 적용
□ 소제목 5개 이상
□ 인용 가능한 팩트 문장 3개 이상
