...
function getCurrentContinuousImageSourceForSafety(): string {
  try {
 ...
 *   - 5~7분 설정: jitter + 심야 + 프로그레시브 + 쿨다운 (보호 필요)
 *   - 5분 미만: 최소 5분으로 강제 + 풀 방어
...
export function initContinuousPublishingV2(): void {
      ...
      if (editingIndex >= 0) {
        ...
        if (item) {
          ...
          continuousQueueV2[editingIndex] = {
            ...
            interval: normalizeSafePublishInterval(parseInt(intervalValueInput?.value || '30') * parseInt(intervalUnitSelect?.value || '1')),
            realCategory,
            realCategoryName,
            scheduleDate,
  ...
  if (continuousContentModeSelect) {
    ...
    // ✅ [v1.4.22] 서비스 범위 라디오 변경 시 지역 필드 토글
    const updateServiceArea = () => {
      const nationwide = (document.getElementById('business-service-nationwide') as HTMLInputElement)?.checked;
      const regionInput = document.getElementById('business-info-region') as HTMLInputElement;
      if (regionInput) {
        if (nationwide) {
          regionInput.disabled = true;
          regionInput.placeholder = '🌏 전국 서비스 — 지역 입력 불필요';
          regionInput.value = '';
          regionInput.style.opacity = '0.4';
        } else {
          regionInput.disabled = false;
          regionInput.placeholder = '🗺️ 지역 (예: 부산, 울산 / 강남, 송파)';
          regionInput.style.opacity = '1';
        }
      }
    };
    document.getElementById('business-service-nationwide')?.addEventListener('change', updateServiceArea);
    document.getElementById('business-service-regional')?.addEventListener('change', updateServiceArea);
  ...
  document.querySelectorAll('input[name="continuous-modal-shopping-subimage-source"]').forEach(radio => {
    radio.addEventListener('change', (e) => {
      ...
      // ✅ [v2.10.335] 나노바나나 3종 분리 — 통합 정규화 제거, 3종 모두 AI 엔진으로 인식
...
function addItemToQueueV2Impl(): void {
  ...
  for (const val of inputValues) {
    ...
    const newItem: ContinuousQueueItem = {
      ...
      imageSource,
      interval: normalizeSafePublishInterval(interval), // 장기 발행 안전 간격 적용, 최대 24시간
      publishMode,
  ...
  try {
    ...
    if (!textOnlyGuard) {
      const guardSources = Array.from(new Set(
        pendingItems
          .map((item: any) => String(item.imageSource || '').trim())
          .filter((source: string) => !!source && source !== 'skip' && source !== 'saved')
      ));
      if (guardSources.length === 0) guardSources.push(UnifiedDOMCache.getImageSource());

      for (const guardImageSource of guardSources) {
        const passed = await runOpenAIImageGuard(guardImageSource);
        if (!passed) {
          appendLog(`⛔ 이미지 엔진 사전 가드 차단 — 연속발행 중단 (${guardImageSource})`);
          return;
        }
      }
    }
  ...
  for (let i = 0; i < continuousQueueV2.length; i++) {
    ...
    if (nextPending && isContinuousMode) {
      ...
      if (isScheduleMode) {
        ...
        appendLog(`⏰ 예약 발행 모드 → 생성/로그인 보호를 위해 ${safeWait}초(${Math.round(safeWait/60)}분) 후 다음 항목 처리...`);
...
async function processNextInQueueEnhanced(): Promise<void> {
  ...
  // ✅ [2026-03-20] 안전 발행 간격 적용 (캡차 방지)
  // 예약 발행도 앱 내부 콘텐츠/이미지 생성과 로그인 작업이 이어지므로 안전 대기 적용
  if (continuousPublishQueue.length > 0) {
    ...
    try {
      ...
      if (isScheduleMode) {
        ...
        const userInterval = (parseInt(intervalValEl?.value || '7') || 7) * (parseInt(intervalUnitEl?.value || '60') || 60);
        const result = getSafePublishInterval(userInterval, _continuousPublishCount);
        appendLog(`⏰ 예약 발행 모드 → 생성/로그인 보호를 위해 ${result.interval}초(${Math.round(result.interval/60)}분) 후 다음 항목 처리...`);
      ...
      // 스케줄 계산 실패 — 큐를 멈추지 않고 안전 폴백 간격으로 다음 항목 처리
      const fallbackInterval = getImageAwareSafePublishFloorSec();
      appendLog(`⚠️ 발행 간격 계산 오류 — 안전 폴백(${fallbackInterval}초) 후 다음 항목 처리: ${(scheduleError as Error).message}`);
      _enhancedQueueTimer = setTimeout(() => { _enhancedQueueTimer = null; processNextInQueueEnhanced(); }, fallbackInterval * 1000);
    }
  } else {
    appendLog('✅ 모든 연속 발행 완료!');
    stopContinuousMode('complete');
