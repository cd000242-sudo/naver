...
export async function handleFullAutoPublish(): Promise<void> {
  ...
  try {
    ...
    if (structuredContent) {
      ...
      // ✅ [v2.11.49] 지연 경고 배너 제거 — 진행률 바가 이미 작동 상태를 보여주므로
      //   주황색 ⚠️ 배너는 에러처럼 보여 사용자 혼란. 조용한 로그만 남기고 임계값도 5분으로 상향(이미지 생성은 흔히 2분 초과).
      const contentTimeout = setTimeout(() => {
        appendLog('⏳ 생성이 조금 오래 걸리고 있어요 — 계속 진행 중입니다.');
      }, 300_000);
    ...
    if (!skipImages) {
      ...
      modal.addLog('♻️ 이전 이미지 배치 제거 완료 — 새 이미지로 다시 배치합니다.');
      modal.showImages?.([], '♻️ 이미지 새로 생성 준비 중...');
    } else {
      (window as any).clearFullAutoGeneratedImageBatch?.(structuredContent, 'skip-images');
    }
    ...
    if (!skipImages) {
      ...
      try {
        ...
        (window as any).isGeneratingImages = false;
        if ((window as any).isFullAutoImageRunCurrent?.(fullAutoImageRunId, imageSource)) {
          (window as any).clearFullAutoGeneratedImageBatch?.(structuredContent, 'image-error');
          modal.showImages?.([], '⚠️ 이미지 실패 — 이전 이미지 배치 제거 완료');
        }
