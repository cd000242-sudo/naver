...
export function describeNaverFailure(status: number, mode: NaverApiMode): string {
  ...
  /*
   * [2026-09-15 실측] 이 문구가 오진을 만들었다. 짧은 시간에 72회를 몰아 부르자 뉴스·지식iN 이 429 를 냈고,
   * 몇 분 뒤 같은 키로 모든 엔드포인트가 200 이었다. 즉 429 는 **일일 쿼터 소진만이 아니라 순간 호출 속도**로도 난다.
   * 그런데 문구가 "일일 쿼터를 넘겼습니다" 라고 단정해, 자료가 안 붙는 원인을 하루치 한도로 오해하게 만들었다.
   * 둘을 구분할 정보가 응답에 없으므로 **단정하지 않고 둘 다 말하고, 확인 방법을 준다.**
   */
  if (status === 429) {
    return mode === 'hub'
      ? `호출 한도 초과 (429) — API HUB 는 Application 에서 해당 서비스를 선택하지 않아도 429 가 납니다. `
        + `콘솔에서 서비스 선택 여부와 호출 한도를 함께 확인하세요.`
      : `호출 한도 초과 (429) — 순간 호출이 몰렸거나 하루 한도(25,000회)를 넘긴 경우입니다. `
        + `잠시 뒤 다시 되면 속도 문제이고, 계속 429 면 하루 한도입니다. 같은 계정의 다른 키로 바꿔도 결과는 같습니다.`;
  }
