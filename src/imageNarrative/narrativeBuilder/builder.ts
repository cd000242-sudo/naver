...
function buildUserPrompt(plan: NarrativePlan, options: BuilderOptions): string {
  ...
  return (
    ...
    `- [소제목] 각 섹션 heading 은 **그 장소가 어떤 곳이었는지가 함축된** 문장형(12~30자)으로 쓸 것.\n` +
    ...
    `  · 끝맺음은 둘 중 하나 — (가) 정확한 종결("우산을 양산 삼아 근포땅굴에 닿았어요") ` +
    `또는 (나) 여운 있는 체언 종결("10분 기다려 받은 꼬막 한 상")\n` +
    `  · 모든 소제목을 같은 종결로 통일하지 말고 (가)·(나)를 섞을 것\n` +
