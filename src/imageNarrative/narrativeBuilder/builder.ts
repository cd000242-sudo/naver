...
function buildUserPrompt(plan: NarrativePlan, options: BuilderOptions): string {
  ...
  return (
    ...
    // [2026-09-09 사장님] "그냥 일기장 쓴 느낌, ~했어요 ~했습니다만 있으니 밋밋해.
    //   감탄사나 '와...' 이런 거 있잖아. 감정이 담겨 있어야지." — 사실 서술만 나열하면
    //   사람이 쓴 글로 안 읽힌다. 반응을 문장으로 요구한다(사진에 있는 것에 대한 반응만).
    `- [감정 반응] 사실만 나열하지 말 것. 사진 속 장면을 보고 그때 든 반응을 문장으로 남길 것.\n` +
    `  · 감탄·놀람·아쉬움·설렘 같은 반응을 섹션마다 최소 1번은 넣는다 ` +
    `(예: "와, 이건 진짜 예상 못 했어요.", "솔직히 여기서 좀 멈칫했어요.", "이 순간이 제일 좋았어요.")\n` +
    `  · 좋았던 것만 쓰지 말 것. 아쉬웠던 점·망설였던 순간·예상과 달랐던 부분을 최소 1군데는 솔직하게 적는다.\n` +
    `  · 다만 감탄사를 문단마다 반복하지 말 것. "와" "헐" "대박" 을 글 전체에서 3번 넘게 쓰지 않는다.\n` +
    `  · 반응의 근거는 반드시 사진에 보이는 것이어야 한다 — 감정을 위해 없는 사실을 만들지 말 것.\n` +
    // [2026-09-09 사장님] "소제목도 내 경험과 어우러져 문장형으로 깔끔하게",
    //   "문장형으로 하되 어색하면 안 된다고, 끝맺음은 정확하거나 여운이 남게".
    //   실측 발행글의 소제목이 "근포땅굴 도착 / 점심 꼬막 한상" 같은 명사 라벨이었다.
    // [2026-09-09 사장님] "소제목은 대충 짓는 게 아니라 함축적으로 그 장소에 대한
    //   의미가 담겨 있어야 돼." 실측 소제목 "초록빛 동굴을 뒤로한 점심 한 상" 은
    //   꼬막집 섹션인데 앞 장소(동굴)를 끌어와 그 장소가 어떤 곳인지 보이지 않았다.
    // [2026-09-09 사장님] "'…근포마을을 마주했어요' 보다 '…마주한 근포마을' 이런 소제목이
    //   낫지 않니?" 종결어미는 늘어지고, 추출기(clearSentenceEnding)가 소제목으로 보지도 않는다.
    `- [소제목] 각 섹션 heading 은 **그 장소가 어떤 곳이었는지가 함축된** 한 마디(12~30자)로 쓸 것.\n` +
    `  · 그 섹션 사진들이 공통으로 말하는 것 하나를 잡는다. 앞 섹션 장소를 끌어오지 말 것 ` +
    `(나쁨: "초록빛 동굴을 뒤로한 점심 한 상" — 꼬막집 소제목에 앞 장소를 끌어왔다)\n` +
    `  · 어디에 붙여도 말이 되는 분위기 낱말 나열은 실패 (나쁨: "빛과 바다가 어우러진 순간")\n` +
    `  · 시간·개수·금액은 아라비아 숫자로 (나쁨: "열 분 기다려" / 좋음: "10분 기다려")\n` +
    `  · 끝맺음은 명사(체언)로. 종결어미로 문장을 맺지 말 것 ` +
    `(나쁨: "부둣가 주차장에서 근포마을을 마주했어요" / 좋음: "부둣가 주차장에서 마주한 근포마을")\n` +
    `  · 관형형(~한/~된/~는/~하던) + 명사 꼴이 가장 잘 맞는다\n` +
    `  · 끝맺음만 명사로 통일하고, 여는 말은 장소·행동·숫자로 섞을 것 (짜임을 찍어내지 말 것)\n` +
    `  · 명사 라벨("근포땅굴 도착"), 번역투, 광고 문구, 정보성 상투구는 금지. 어색하면 다시 쓸 것\n\n` +
