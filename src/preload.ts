...
contextBridge.exposeInMainWorld('api', {
  ...
  collectIssueImages: (payload: { title: string; headings: Array<{ title: string; body?: string }>; mainKeyword?: string; intro?: string; paidVisionCheck?: boolean }): Promise<{ success: boolean; images: Record<string, string[]>; candidates: Record<string, any[]>; stats?: any; message?: string }> =>
  ...
  // [포커스 가드] 네이티브 팝업 닫힘 후 입력 라우팅 리셋 (dialogFocusGuard와 한 쌍)
  refocusWindow: (): Promise<{ success: boolean }> => ipcRenderer.invoke('window:refocus'),
