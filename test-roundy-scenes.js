/**
 * 뚱글이(Roundy) 스타일 이미지 생성 테스트
 * - DeepInfra FLUX-2-dev API
 * - 소제목 6개: 자영업자 고용보험 관련 블로그 글
 * - ✅ deepinfraGenerator.ts의 프롬프트 조합 구조 그대로 사용
 *   → TEXTLESS ${basePrompt}, character style: ${roundyStyle}. SINGLE ILLUSTRATION ONLY...
 */
...
function buildPrompt(basePrompt) {
    return `TEXTLESS ${basePrompt}, character style: ${ROUNDY_STYLE}. SINGLE ILLUSTRATION ONLY, NOT a comic strip, NEVER SPEECH BUBBLES, NEVER DIALOGUE, CLEAN IMAGE ONLY.`;
}
...
async function runTests() {
    console.log('═══════════════════════════════════════════');
    console.log('🫧 뚱글이(Roundy) 스타일 이미지 생성 테스트');
    console.log('   소제목 6개 × 뚱글이 스타일');
    console.log('   프롬프트: basePrompt(장면) → character style(뚱글이)');
    console.log('═══════════════════════════════════════════');
