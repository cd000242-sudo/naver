...
// Content policy is a runtime contract. Keep the packaged copy byte-equal to
// the repository default so development and installed builds use one policy.
const policySourceDir = path.join(projectRoot, 'config');
const policyTargetDir = path.join(projectRoot, 'dist', 'config');
try {
  await access(path.join(policySourceDir, 'content_policy.yaml'));
  await mkdir(policyTargetDir, { recursive: true });
  await cp(policySourceDir, policyTargetDir, { recursive: true, force: true });
  console.log('📦 Copied content policy to dist/config');
} catch (e) {
  console.warn('⚠️ content policy copy failed:', e.message);
}
...
try {
  ...
  const utilsModules = [
    'browserPreviewBridge.js',
    // 기본 유틸리티 (의존성 없음)
    'pipelineRunCoordinator.js',
    'hashtagUtils.js',
...
`;
  ...
  // ✅ APP_VERSION inject (renderer.ts:3 'runtime/version.generated.js' import는 inline 누락 →
  //   require → null 치환 + version_generated_js_1.APP_VERSION → APP_VERSION 치환 후 미정의)
  let appVersion = 'unknown';
  try {
    const versionFilePath = path.join(projectRoot, 'src', 'runtime', 'version.generated.ts');
    const versionContent = await readFile(versionFilePath, 'utf-8');
    const m = versionContent.match(/APP_VERSION\s*=\s*['"]([^'"]+)['"]/);
    if (m) appVersion = m[1];
  } catch {
    // 누락 시 'unknown'으로 fallback (renderer 동작은 유지, 버전 표시만 unknown)
    console.warn('⚠️ src/runtime/version.generated.ts not found — APP_VERSION="unknown"');
  }

  const finalRenderer = `// Auto-generated by copy-static.mjs to run inside the renderer (ES module context)
...
${sanitized}`;
  ...
  const missingRuntimeSymbols = REQUIRED_RENDERER_RUNTIME_SYMBOLS.filter((symbol) => {
    ...
    return !new RegExp(`\\bfunction\\s+${escaped}\\s*\\(`).test(finalRenderer);
  });
  ...
  //   renderer 곳곳에서 `(window as any).functionName` 같은 동적 lookup이 있어
  //   identifier mangling 시 깨질 위험. 식별자 보존하면 안전 + ~30-40% 크기 감소.
  let toWrite = finalRenderer;
  if (process.env.ECC_SKIP_MINIFY !== '1') {
    try {
      const { transform } = await import('esbuild');
      const beforeBytes = Buffer.byteLength(finalRenderer, 'utf-8');
      const result = await transform(finalRenderer, {
        loader: 'js',
      ...
      toWrite = result.code;
      const afterBytes = Buffer.byteLength(toWrite, 'utf-8');
      const pct = ((1 - afterBytes / beforeBytes) * 100).toFixed(1);
      console.log(`📦 [v2.10.80] esbuild minify: ${(beforeBytes / 1024).toFixed(0)}KB → ${(afterBytes / 1024).toFixed(0)}KB (-${pct}%)`);
    } catch (mErr) {
      console.warn('⚠️ esbuild minify 실패 — 원본 코드 그대로 사용:', mErr.message);
      toWrite = finalRenderer;
    }
  } else {
