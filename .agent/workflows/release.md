...
> ✅ 단일 오케스트레이터 (`release-all.js`)가 아래 6단계를 자동 수행:
> 1. `npm run build` (tsc 컴파일)
> 2. `reset-config-for-pack.js` (민감 정보 제거)
> 3. `electron-builder --win --config.win.target=nsis` (Setup.exe 생성)
> 4. `restore-after-pack.js` (민감 정보 복원)
> 5. `fix-latest-yml.js` (latest.yml SHA512 재계산)
> 6. `upload-release.js` (git push → 릴리즈 → 업로드 → 검증)
>
> ⚠️ 어디서 실패하든 민감정보 자동 복구 (finally 패턴)
> ⚠️ 실패 시 `release_final/release-error.log`에 상세 진단 로그 보존
...
| `Setup 파일 없음` | 빌드 실패 | `release_final/release-error.log` 확인 |
...
| 실패 후 renderer.ts 변경됨 | 복원 실패 | `git checkout src/renderer/renderer.ts` |
